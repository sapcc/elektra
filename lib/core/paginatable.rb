  ...
  module Paginatable
    ...
    module InstanceMethods
      def paginatable(options = { per_page: 20 }, &block)
        ...
        @pagination_seen_items =
          (@pagination_current_page - 1) * @pagination_per_page

        sort_order_mode = {
          normal: options[:reverse] ? "asc" : "desc",
          reverse: options[:reverse] ? "desc" : "asc",
        }
        ...
        pagination_options = {
          ...
          sort_dir:
            (
              if (params[:reverse] && @pagination_current_page > 1)
                sort_order_mode[:normal]
              else
                sort_order_mode[:reverse]
              end
            ),
        }
        pagination_options[:marker] = params[:marker] if (
          params[:marker] && @pagination_current_page > 1
        )
