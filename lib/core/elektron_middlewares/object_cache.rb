    ...
    class ObjectCache < ::Elektron::Middlewares::Base
      ...
      def find_objects(data, parent_key = nil)
        ...
        if data.is_a?(Hash)
          ...
          if parent_key == "role_assignments"
            ...
            begin
              ...
              id =
                url
                  .scan(
                    %r{(?:projects/([^/]+)|users/([^/]+)|roles/([^/]+)|groups/([^/]+)|domains/([^/]+))},
                  )
                  .flatten
                  .compact
                  .join("-")
              type =
                url
                  .scan(/(?:project|user|role|group|domain)/)
                  .flatten
                  .compact
                  .join("_") + "_assignment"
              ...
              if role_assignment.key?("scope")
                role_assignment["origin_scope"] = role_assignment.delete(
                  "scope",
                )
              end

              if role_assignment["origin_scope"] &&
                   role_assignment["origin_scope"]["domain"]
                role_assignment["domain_id"] = role_assignment["origin_scope"][
                  "domain"
                ][
                  "id"
                ]
              ...
              if role_assignment["origin_scope"] &&
                   role_assignment["origin_scope"]["project"] &&
                   role_assignment["origin_scope"]["project"]["domain"]
                role_assignment["domain_id"] = role_assignment["origin_scope"][
                  "project"
                ][
                  "domain"
              ...
              if role_assignment["origin_scope"] &&
                   role_assignment["origin_scope"]["project"]
                role_assignment["project_id"] = role_assignment["origin_scope"][
                  "project"
                ][
                  "id"
                ]
          ...
          objects =
            data.each_with_object([]) do |hash, array|
