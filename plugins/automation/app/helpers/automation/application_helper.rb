module Automation
  module ApplicationHelper
    ...
    def form_horizontal_json_editor(
      ...
      content_tag :div, class: "form-group #{has_error ? "has-error" : ""}", id: id do
        concat content_tag :label, label, class: "text col-sm-4 control-label #{label_classes}"
        concat (content_tag :div, class: "col-sm-8" do
          ...
          unless help_text.blank?
            concat (content_tag :p, class: "help-block" do
              concat tag :i, class: "fa fa-info-circle"
              concat help_text
            end)
          end
          if has_error
            object.errors.messages[attribute_name.to_sym].collect do |message|
