...
(function($){
  ...
  $.fn.tagEditor = function(options, val, blur){
    ...
    o.kvdregex = new RegExp('['+o.keyValueDelimiter.replace('-', '\-')+']', 'g');
    ...
    return selector.each(function(){
      ...
      // create the key value par
      function create_key_value(tag){
        var key_value = tag.split(o.kvdregex);
      ...
      function check_duplicate($this, tag){
        var key_value = create_key_value(tag)
        if (key_value.length == 0) {
          return false
        }
      ...
      function split_cleanup(input){
        ...
        for (var i=0; i<sub_tags.length; i++) {
          ...
          if (o.removeDuplicates && ~$.inArray(tag, old_tags))
            $('.tag-editor-tag', ed).each(function(){
              if (check_duplicate($(this), tag)) $(this).closest('li').remove();
            });
          key_value_tag = create_tag(tag);
          if (key_value_tag == false) continue;
          old_tags.push(tag);
          li.before('<li><div class="tag-editor-spacer">&nbsp;'+o.delimiter[0]+'</div><div class="tag-editor-tag">'+key_value_tag+'</div><div class="tag-editor-delete"><i></i></div></li>');
      ...
      ed.on('blur', 'input', function(e){
        ...
        key_value_tag = create_tag(tag);
        if (key_value_tag == false) {
          var li = $(this).closest('li')
          li.remove();
          set_placeholder();
        }
        input.parent().html(create_tag(tag)).removeClass('active');
      ...
      for (var i=0; i<tags.length; i++) {
        ...
        if (tag) {
          if (o.forceLowercase) tag = tag.toLowerCase();
          tag_list.push(tag);
          ed.append('<li><div class="tag-editor-spacer">&nbsp;'+o.delimiter[0]+'</div><div class="tag-editor-tag">'+create_tag(tag)+'</div><div class="tag-editor-delete"><i></i></div></li>');
        }
      }
