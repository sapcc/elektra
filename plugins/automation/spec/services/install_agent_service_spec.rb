...
RSpec.describe InstallAgentService do
  ...
  describe 'process_request' do
    ...
    it 'should raise an exception if instance_id is empty or nil' do
      # empty
      expect { @service.process_request('', '', '', nil, nil, '', '') }.to raise_error(InstallAgentParamError)
      # blank
      expect { @service.process_request(nil, '', '', nil, nil, '', '') }.to raise_error(InstallAgentParamError)
    end
    ...
    context 'compute instance' do
      ...
      it 'should raise an exception if instance not found' do
        ...
        expect { @service.process_request(@instance_id, @instance_type, '', compute_service, nil, '', '') }.to raise_error(InstallAgentParamError)
      end
      ...
      it "should not raise an error if agent already exists on the instance" do
        ...
        expect { @service.process_request(@instance_id, @instance_type, '', compute_service, @automation_service, @active_project, '') }.to raise_error(InstallAgentInstanceOSNotFound)
      end
      ...
      it "should raise an exception if instance_os and image metadata os_family are empty or nil" do
        ...
        expect { @service.process_request(@instance_id, @instance_type,  '', compute_service, @automation_service, '', '') }.to raise_error(InstallAgentInstanceOSNotFound, "Instance OS empty or not known")
        expect { @service.process_request(@instance_id, @instance_type, nil, compute_service, @automation_service, '', '') }.to raise_error(InstallAgentInstanceOSNotFound, "Instance OS empty or not known")
      end
