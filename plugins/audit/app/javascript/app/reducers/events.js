...
export const events = function(state, action) {
  if (state == null) { state = initialEventState; }
  switch (action.type) {
    case constants.REQUEST_EVENTS:                     return requestEvents(state,action)
    case constants.RECEIVE_EVENTS:                     return receiveEvents(state,action)
    case constants.REQUEST_EVENTS_FAILURE:             return requestEventsFailure(state,action)
    case constants.REQUEST_ATTRIBUTE_VALUES:           return requestAttributeValues(state,action)
    case constants.RECEIVE_ATTRIBUTE_VALUES:           return receiveAttributeValues(state,action)
    case constants.REQUEST_ATTRIBUTE_VALUES_FAILURE:   return requestAttributeValuesFailure(state,action)
    case constants.REQUEST_ATTRIBUTE_VALUES_NOT_FOUND: return requestAttributeValuesNotFound(state,action)
    case constants.UPDATE_FILTER_START_TIME:           return updateFilterStartTime(state,action)
    ...
    case constants.REQUEST_EVENT_DETAILS:              return requestEventDetails(state,action)
    case constants.REQUEST_EVENT_DETAILS_FAILURE:      return requestEventDetailsFailure(state,action)
    case constants.RECEIVE_EVENT_DETAILS:              return receiveEventDetails(state,action)
