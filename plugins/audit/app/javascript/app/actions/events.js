...
const buildTimeFilter = (filterStartTime, filterEndTime) => {
  ...
  if(filterStartTime != null && (moment.isMoment(filterStartTime) || !isEmpty(filterStartTime)))
    timeFilter += `gte:${filterStartTime.format("YYYY-MM-DD[T]h:mm:ss")}`

  if(filterEndTime != null && (moment.isMoment(filterEndTime) || !isEmpty(filterEndTime)))
    timeFilter += `${timeFilter.length > 0 ? ',' : ''}lte:${filterEndTime.format("YYYY-MM-DD[T]h:mm:ss")}`
...
const updateOffset = (offset) => (
  { type: constants.UPDATE_OFFSET, offset }
)

const updateCurrentPage = (page) => (
  { type: constants.UPDATE_CURRENT_PAGE, page: page }
)
...
const updateFilterStartTime = (filterStartTime) => (
  { type: constants.UPDATE_FILTER_START_TIME, filterStartTime }
)
...
const updateFilterEndTime = (filterEndTime) => (
  {type: constants.UPDATE_FILTER_END_TIME,filterEndTime }
)
...
const requestAttributeValuesFailure = (error) => (
  {type: constants.REQUEST_ATTRIBUTE_VALUES_FAILURE, error}
)

const requestAttributeValuesNotFound = (attribute) => (
  {type: constants.REQUEST_ATTRIBUTE_VALUES_NOT_FOUND,attribute}
)

const receiveAttributeValues = (attribute, json) => (
  {type: constants.RECEIVE_ATTRIBUTE_VALUES,values: json,attribute}
)

const fetchAttributeValues = (attribute) =>
  (dispatch) => dispatch(loadAttributeValues(attribute))

const loadAttributeValues = (attribute) =>
  (dispatch, getState) => {
    const currentState    = getState()
    const events          = currentState.events
    ...
    return ajaxHelper.get(`/attributes/${attribute}`).then( (response) => {
      if (response.data.errors) {
        addError(React.createElement(ErrorsList, {errors: response.data.errors}))
      } else {
...
export const toggleEventDetails = (event) =>
  (dispatch) => {
...
const loadEventDetails = (event) =>
  (dispatch, getState) => {
    ...
    return ajaxHelper.get(`/events/${event.id}`).then( (response) => {
      if (response.data.errors) {
        addError(React.createElement(ErrorsList, {errors: response.data.errors}))
      } else {
...
const requestEventDetails = (event) => (
  {type: constants.REQUEST_EVENT_DETAILS,eventId: event.id}
)
