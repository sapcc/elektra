...
module EmailService
  ...
  class TemplatedEmailsController < ::EmailService::ApplicationController
    ...
    TEMPLATED_EMAIL_SENT =
      I18n.t('email_service.messages.templated_email_sent').to_s.freeze
    ...
    def create
      ...
      case @templated_email.source_type
      when 'domain'
        @templated_email.source =
          if @templated_email.source_domain_name_part.nil?
      ...
      @templated_email.return_path =
        @templated_email.source if @templated_email.return_path == ''

      if @templated_email.tags.empty?
        @templated_email.tags = [{ name: 'name1', value: 'value1' }]
      end

      templated_email_values =
        @templated_email.process(EmailService::TemplatedEmail)
      unless @templated_email.valid?
        return render action: 'edit', locals: { data: { modal: true } }
      end
