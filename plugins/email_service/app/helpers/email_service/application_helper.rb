module EmailService
  module ApplicationHelper
    ...
    def batch_get_metric_data(
      ...
      begin
        resp =
          ses_client_v2.batch_get_metric_data(
            {
              queries: [ # required
                {
        ...
        error =
          "[email_service][application_helper][batch_get_metric_data] #{e.message}"
        Rails.logger.error "\n #{error} \n"
      rescue Exception => e
        error =
          "[email_service][application_helper][batch_get_metric_data] #{e.message}"
    ...
    def list_email_identities(nextToken = "", page_size = 1000)
      ...
      if nebula_active? && ec2_creds && ses_client_v2
        resp =
          ses_client_v2.list_email_identities(
            { next_token: nextToken, page_size: page_size },
          )
        ...
        resp.email_identities.each do |item|
          ...
          unless item.identity_name.include?(
            ...
            details =
              ses_client_v2.get_email_identity(
                {
            ...
            identity.merge!(
              {
                ...
                verified_for_sending_status:
                  details.verified_for_sending_status, #=> Boolean
                dkim_attributes: details.dkim_attributes, #=> Boolean
                mail_from_attributes:
                  details.mail_from_attributes.mail_from_domain, #=> String
    ...
    def create_email_identity_email(
      ...
      begin
        ...
        flash[
          :error
        ] = "[create_email_identity_email] : Status Code:[500]: #{e.message} "
        status = e.message
      end
      return status
    end
    ...
    def delete_email_identity(identity)
      status = nil
      begin
        resp = ses_client_v2.delete_email_identity({ email_identity: identity })
        audit_logger.info(
          current_user.id,
    ...
    # TO_DO
    def domain_statistics_report(
      identity,
      report_start_date = Time.now - 86_400,
      report_end_date = Time.now
    )
      begin
        @domain_statistics_report =
          ses_client_v2.get_domain_statistics_report(
            {
    ...
    def list_contact_lists(next_token = "", page_size = 1)
      begin
        @resp =
          ses_client_v2.list_contact_lists(
    ...
    def put_email_identity_dkim_signing_attributes(
      email_identity,
      signing_attributes_origin = "AWS",
      signing_attributes = {}
    )
      resp =
        ses_client_v2.put_email_identity_dkim_signing_attributes(
          {
    ...
    def put_email_identity_configuration_set_attributes(
      ...
      begin
        resp =
          ses_client_v2.put_email_identity_configuration_set_attributes(
            {
              email_identity: "Identity", # required
        ...
        status =
          "[put_email_identity_configuration_set_attributes] : exception : #{e.message}"
    ...
    def start_dkim_verification(identity)
      ...
      begin
        resp = ses_client.verify_domain_dkim({ domain: identity })
        audit_logger.info(
          current_user.id,
          " has initiated DKIM verification ",
          identity,
        )
        status = "success"
      rescue Aws::SES::Errors::ServiceError => e
    ...
    def enable_dkim(identity)
      ...
      begin
        resp =
          ses_client.set_identity_dkim_enabled(
            { dkim_enabled: true, identity: identity },
          )
    ...
    def disable_dkim(identity)
      ...
      begin
        resp =
          ses_client.set_identity_dkim_enabled(
            { dkim_enabled: false, identity: identity },
          )
    ...
    def send_templated_email(templated_email)
        ...
        send_email_hash.merge!(
          {
            list_management_options: {
              contact_list_name:
                templated_email.list_management_options.contact_list_name, # "ContactListName", # required
      ...
      begin
        ...
        status =
          "success - email sent to #{templated_email.to_addr} ,#{templated_email.cc_addr}, #{templated_email.bcc_addr}"
        audit_logger.info(
          current_user.id,
    ...
    def list_templates(token = nil)
      ...
      begin
        template_list =
          ses_client.list_templates({ next_token: token, max_items: 10 })
        next_token = template_list.next_token
        index = 0
        while template_list.size > 0 &&
                index < template_list.templates_metadata.count
          resp =
            ses_client.get_template(
    ...
    def delete_template(template_name)
      ...
      begin
        resp = ses_client.delete_template({ template_name: template_name })
        audit_logger.info(
          current_user.id,
          "has deleted template ",
          template_name,
        )
        status = "success"
      rescue Aws::SES::Errors::ServiceError => e
    ...
    def toggle_dkim(identity, dkim_enabled = true)
      ...
      begin
        resp =
          ses_client_v2.put_email_identity_dkim_attributes(
            {
              email_identity: identity, # required
    ...
    def store_configset(configset)
      begin
        resp =
          ses_client_v2.create_configuration_set(
            {
              ...
              reputation_options: {
                reputation_metrics_enabled:
                  configset.reputation_metrics_enabled,
    ...
    def delete_configset(name)
      begin
        resp =
          ses_client_v2.delete_configuration_set(
    ...
    def delete_custom_verification_email_template(name)
      begin
        resp =
          ses_client.delete_custom_verification_email_template(
            { template_name: name },
          )
        return "success"
      rescue Aws::SES::Errors::ServiceError => e
    ...
    def update_custom_verification_email_template(custom_template = {})
      begin
        resp =
          ses_client.update_custom_verification_email_template(
            {
    ...
    def custom_templates(next_token = nil)
      ...
      resp =
        list_custom_verification_email_templates(
          { next_token: next_token, max_results: 50 },
        )

      return resp if resp.is_a?(Error)
    ...
    def list_custom_verification_email_templates(params = {})
      begin
        @all_templates ||=
          ses_client.list_custom_verification_email_templates(params)
      rescue Aws::SES::Errors::ServiceError => e
        status = "#{e.message}"
    ...
    def find_custom_verification_email_template(name)
      begin
        template =
          ses_client.get_custom_verification_email_template(
            {
    ...
    def nebula_status
      if nebula_details
        if nebula_details.is_a?(Hash)
          ...
          elsif nebula_details.has_key?("compliant") &&
                nebula_details.has_key?("security_attributes")
            status = "REQUESTED"
          else
            status = "INACTIVE"
          end
          ...
          begin
            return JSON.parse(nebula_details).has_key?("error") ? "ERROR" : ""
    ...
    def nebula_active?
      @nebula_active ||=
        nebula_details && nebula_details["status"] == "GRANTED" ? true : false
    end
    ...
    def nebula_activate(multicloud_account = nil)
      ...
      endpoint_url =
        (
          if multicloud_account.custom_endpoint_url
