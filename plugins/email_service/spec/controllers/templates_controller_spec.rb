...
describe EmailService::TemplatesController, type: :controller do
  ...
  before :each do
    ...
    allow_any_instance_of(EmailService::TemplatesController).to receive(
    ...
    allow_any_instance_of(EmailService::TemplatesController).to receive(
      :list_templates,
    ).and_return(double("templates").as_null_object)
    allow_any_instance_of(EmailService::TemplatesController).to receive(
      :delete_template,
    ).and_return(double("status").as_null_object)
    allow_any_instance_of(EmailService::TemplatesController).to receive(
      :find_template,
    ).and_return(double("template").as_null_object)
    allow_any_instance_of(EmailService::TemplatesController).to receive(
      :get_all_templates,
    ).and_return(double("templates").as_null_object)
    allow_any_instance_of(EmailService::TemplatesController).to receive(
      :get_templates_collection,
    ).and_return(double("temp_collection").as_null_object)
  end
  ...
  describe "DELETE 'destroy'" do
    ...
    context "email_admin" do
      ...
      it "returns http 302 status" do
        ...
        ).to redirect_to(templates_path(default_params))
