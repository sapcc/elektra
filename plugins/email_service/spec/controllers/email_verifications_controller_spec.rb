...
describe EmailService::EmailVerificationsController, type: :controller do
  ...
  before :each do
    ...
    allow_any_instance_of(EmailService::EmailVerificationsController).to receive(:check_verified_identity).and_return(double('render').as_null_object)
  ...
  describe "POST 'create'" do
    ...
    context 'cloud_support_tools_viewer_role alone' do
      ...
      it 'returns http 401 status' do
        expect(post(:create, params: default_params.merge(verified_email: @opts))).to render_template('application/exceptions/warning.html')
      end
    end
