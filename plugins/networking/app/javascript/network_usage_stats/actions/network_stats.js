...
const requestNetworkUsageStatsFailure = () => ({
  type: constants.REQUEST_NETWORK_USAGE_STATS_FAILURE,
})
      ...
      ajaxHelper
        ...
        .then((response) => {
          if (response.data.errors) {
            ...
            dispatch(
              receiveNetworkUsageStats(response.data.network_usage_stats)
            )
            handleSuccess()
          }
        })
...
const shouldFetchNetworkUsageStats = function (state) {
  if (
    state.network_usage_stats.isFetching ||
    state.network_usage_stats.requestedAt
  ) {
    return false
  } else {
    return true
...
const fetchNetworkUsageStatsIfNeeded = () =>
  function (dispatch, getState) {
    if (shouldFetchNetworkUsageStats(getState())) {
      return dispatch(fetchNetworkUsageStats())
    } else return Promise.resolve()
  }
export { fetchNetworkUsageStatsIfNeeded }
