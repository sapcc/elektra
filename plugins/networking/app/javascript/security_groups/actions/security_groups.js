...
const fetcSecurityGroups= () =>
  function(dispatch,getState) {
    ...
    return ajaxHelper.get('/security-groups').then( (response) => {
...
const fetchSecurityGroup= (id) =>
  (dispatch) => {
    return new Promise((handleSuccess,handleError) =>
      ajaxHelper.get(`/security-groups/${id}`).then( (response) => {
...
const deleteSecurityGroup=(id) =>
  (dispatch) =>
    confirm(`Do you really want to delete the securit group ${id}?`).then(() => {
      dispatch(requestSecurityGroupDelete(id))
      return ajaxHelper.delete(`/security-groups/${id}`)
...
const submitNewSecurityGroupForm= (values) => (
  (dispatch) =>
    new Promise((handleSuccess,handleErrors) =>
      ajaxHelper.post('/security-groups/', { security_group: values }
      ).then((response) => {
...
const submitEditSecurityGroupForm= (id,values) => (
  (dispatch) =>
    new Promise((handleSuccess,handleErrors) =>
      ajaxHelper.put(`/security-groups/${id}`, { security_group: values }
      ).then((response) => {
        dispatch(receiveSecurityGroup(response.data))
        handleSuccess()
      }).catch(error => handleErrors({errors: errorMessage(error)}))
