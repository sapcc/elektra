...
const deleteSecurityGroupRule = (securityGroupId, id) => (dispatch) =>
  confirm(`Do you really want to delete the rule ${id}?`)
    .then(() => {
      dispatch(requestSecurityGroupRuleDelete(securityGroupId, id))
      return ajaxHelper
        .delete(`/security-groups/${securityGroupId}/rules/${id}`)
        .then((response) =>
          dispatch(removeSecurityGroupRule(securityGroupId, id))
        )
        .catch((error) => {
          addError(
...
const submitNewSecurityGroupRuleForm =
  (securityGroupId, values) => (dispatch) =>
    new Promise((handleSuccess, handleErrors) =>
      ajaxHelper
        .post(`/security-groups/${securityGroupId}/rules`, {
          security_group_rule: values,
        })
        .then((response) => {
...
export { deleteSecurityGroupRule, submitNewSecurityGroupRuleForm }
