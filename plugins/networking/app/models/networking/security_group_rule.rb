...
module Networking
  ...
  class SecurityGroupRule < Core::ServiceLayer::Model
    ...
    def to_s(security_groups = [])
      ...
      result =
        "ALLOW #{ethertype} #{protocol || "any protocol"} #{port} #{direction == "ingress" ? "from" : "to"} "
      result +=
        if remote_ip_prefix.blank? && remote_group_id.blank?
          (ethertype || "").downcase == "ipv4" ? "0.0.0.0/0" : "::/0"
