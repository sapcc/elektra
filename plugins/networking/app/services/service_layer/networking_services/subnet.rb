    ...
    module Subnet
      ...
      def cached_network_subnets(network_id)
        subnets_data = Rails.cache.fetch(
          ...
          elektron_networking.get('subnets', network_id: network_id)
                             .body['subnets']
        end || []
      ...
      def create_subnet(attributes)
        elektron_networking.post('subnets') do
          { 'subnet' => attributes }
        end.body['subnet']
      end

      def update_subnet(id, attributes)
        elektron_networking.put("subnets/#{id}") do
          { 'subnet' => attributes}
        end.body['subnet']
      end
