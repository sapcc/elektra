...
describe Networking::Networks::SubnetsController, type: :controller do
  ...
  describe "POST 'create'" do
    ...
    create_params = subnet_params.select do |k, _v|
    ...
    context 'validation errors' do
      it 'returns json with cidr range errors' do
        post :create, params: default_params.merge(
          network_id: '123', subnet: { name: 'test', cidr: '192.168.199.0/24' }
        )
        expect(response.body).to eq(
          { errors: { cidr: ['must be within the 10.180.0.0/16 range.'] } }.to_json
        )
      end
      ...
      it 'returns json with cidr errors' do
        ...
        expect(response.body).to eq(
          { errors: { cidr: ["can't be blank","must be within the 10.180.0.0/16 range."] } }.to_json
        )
      end

      it 'returns json with name errors' do
        post :create, params: default_params.merge(
          network_id: '123', subnet: { cidr: '10.180.0.0/8' }
        )
        expect(response.body).to eq(
