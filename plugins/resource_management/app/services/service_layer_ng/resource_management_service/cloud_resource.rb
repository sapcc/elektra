module ServiceLayerNg
  ...
  module ResourceManagementService::CloudResource
    ...
    def find_current_cluster(query={})
      Rails.logger.debug  "[resource management-service][CloudResource] -> find_current_cluster -> GET /v1/clusters/current"
      Rails.logger.debug  "[resource management-service][CloudResource] -> find_current_cluster -> Query: #{query}"
      api.resources.get_current_cluster(query).map_to(ResourceManagement::Cluster)
    end

    def put_cluster_data(services)
      Rails.logger.debug  "[resource management-service][CloudResource] -> put_cluster_data -> PUT /v1/clusters/"
