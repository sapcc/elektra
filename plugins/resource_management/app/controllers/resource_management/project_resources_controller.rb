...
module ResourceManagement
  class ProjectResourcesController < ::ResourceManagement::ApplicationController
    ...
    before_action :load_project_resource,
                  only: %i[
                    new_request
                    create_request
                    reduce_quota
    ...
    before_action :check_first_visit,
                  only: %i[index show_area create_package_request]
    ...
    def index
      ...
      if params.include?(:if_updated_since)
        ...
        render json: {
                 sync_running:
                   params[:if_updated_since].to_i > min_updated_at.to_time.to_i,
               }
        return
      end
      ...
      @nearly_full_resources =
        resources.select do |res|
          res.backend_quota.nil? && res.usage > 0 && res.usage > 0.8 * res.quota
        end
    ...
    def show_area(area = nil)
      ...
      @view_services =
        @project.services.select { |srv| srv.area.to_sym == @area }
    ...
    def save_settings
      @project.bursting[:enabled] = params[:project][:bursting_enabled] ==
        "true"
      unless @project.save
        respond_to { |format| format.html { render action: "settings" } }
      end
    end
    ...
    def create_package_request
      ...
      unless pkg
        respond_to do |format|
          format.js do
            render inline:
                     'alert("Error: Invalid quota package name specified.")'
          end
        end
        return
      end
      ...
      # create inquiry
      base_url =
        plugin("resource_management").admin_path(
          domain_id: @scoped_domain_name,
          project_id: nil,
        )
      overlay_url =
        plugin("resource_management").admin_review_package_request_path(
          domain_id: @scoped_domain_name,
          project_id: nil,
        )
      ...
      if inquiry.errors.empty?
        render template:
                 "/resource_management/project_resources/create_package_request.js"
      else
