...
module ResourceManagement
  class CloudAdminController < ::ResourceManagement::ApplicationController
    before_action :load_domain_resource, only: %i[edit cancel update]
    before_action :load_cluster_resource,
                  only: %i[edit_capacity update_capacity cancel update]
    ...
    def show_area(area = nil)
      ...
      @cluster = services.resource_management.find_cluster("current")
      @view_services =
        @cluster.services.select { |srv| srv.area.to_sym == @area }
    ...
    def update_capacity
      ...
      if new_value.blank?
        ...
        begin
          ...
          @cluster_resource.add_validation_error(
            :capacity,
      ...
      render action: :edit_capacity unless @cluster_resource.save
    end
    ...
    def update
      ...
      begin
        @domain_resource.quota =
          @domain_resource.data_type.parse(params.require(:value))
      ...
      unless @domain_resource.save
        render plain: @domain_resource.errors.full_messages.to_sentence,
               status: :bad_request
        return
      end
    ...
    def review_request
      ...
      @maximum_quota_before_overcommit = capacity - domain_quota + quota
    ...
    def approve_request
      ...
      begin
        @desired_quota =
          @domain_resource.data_type.parse(
        ...
        if @desired_quota > @maximum_quota_before_overcommit &&
             !overcommit_accepted
          @domain_resource.add_validation_error(
            :quota,
            "is too large (unless you accept overcommit)",
          )
        end
      rescue ArgumentError => e
        @domain_resource.add_validation_error(
      ...
      if @domain_resource.save
        comment =
          "New domain quota is #{@domain_resource.data_type.format(@domain_resource.quota)}"
    ...
    def details
      ...
      clusters, current_cluster_id =
        services.resource_management.list_clusters(
          service: @service_type.to_s,
          resource: @resource_name.to_s,
          local: true,
        )
      cluster_resources =
        clusters.reject { |c| c.resources.empty? }.map { |c| c.resources.first }

      @cluster_id = current_cluster_id if @cluster_id == "current"
      @cluster_resource =
        cluster_resources.find { |r| r.cluster_id == @cluster_id } or
        raise ActiveRecord::RecordNotFound, "no data for cluster"
      ...
      if @cluster_resource.shared_service?
        @all_cluster_resources =
          cluster_resources.select { |r| r.shared_service? }
        ...
        @combined_resource.domains_quota =
          @all_cluster_resources.map(&:domains_quota).sum
      ...
      domains =
        services.resource_management.list_domains(
          service: @service_type.to_s,
          resource: @resource_name.to_s,
      ...
      @domain_resources =
        Kaminari.paginate_array(@domain_resources).page(params[:page]).per(6)
    ...
    def check_inconsistencies
      ...
      @domain_quota_overcommitted =
        @inconsistencies["domain_quota_overcommitted"]
      ...
      unless sort_by.empty?
        ...
        if @sortable_table == "domain_overcommitted"
          if sort_by == "domain_overcommitted_name"
            @domain_quota_overcommitted.sort_by! do |r|
              [r["domain"]["name"].downcase, r["resource"].downcase]
            end
          else
        ...
        if @sortable_table == "project_overspent" ||
             @sortable_table == "project_mismatch"
          if sort_by == "project_overspent_name" ||
               sort_by == "project_mismatch_name"
            @project_quota_overspent.sort_by! do |r|
              [r["project"]["name"].downcase, r["resource"].downcase]
            end
          else
      ...
      @domain_quota_overcommitted =
        Kaminari
          .paginate_array(@domain_quota_overcommitted)
          .page(params[:page])
          .per(10)
      @project_quota_overspent =
        Kaminari
          .paginate_array(@project_quota_overspent)
          .page(params[:page])
          .per(10)
      @project_quota_mismatch =
        Kaminari
          .paginate_array(@project_quota_mismatch)
          .page(params[:page])
          .per(10)
    end
    ...
    def load_domain_resource
      ...
      domain =
        ...
        raise ActiveRecord::RecordNotFound,
              "domain #{params[:domain]} not found"
    ...
    def load_cluster_resource
      @cluster_id = params[:cluster] || "current"
      cluster =
        services.resource_management.find_cluster(
          @cluster_id,
          service: [params.require(:service)],
          resource: [params.require(:resource)],
        ) or raise ActiveRecord::RecordNotFound, "cluster not found"
      @cluster_resource = cluster.resources.first or
        raise ActiveRecord::RecordNotFound, "resource not found"
    end
    ...
    def load_inquiry
      ...
      enforce_permissions(
        "resource_management:cloud_admin_approve_request",
