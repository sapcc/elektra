...
RSpec.describe ResourceManagement::ResourceBarHelper, type: :helper do
  describe "#resbar_prepare_options" do
    ...
    it "requires some kind of non-negative upper bound" do
      expect {
        call_with(fill: 1, maximum: +1, threshold: -1)
      }.to_not raise_error
      expect {
        call_with(fill: 1, maximum: -1, threshold: +1)
      }.to_not raise_error
    ...
    it "renders custom labels" do
      ...
      expect(result[:threshold]).to include(
        value: 65,
        label: "threshold = (65)",
      )
    end
    ...
    it "recognizes the warning_level option" do
      expect(call_with(fill: 1, warning_level: 0.5)).to include(
        warning_level: 0.5,
      )
    end
  ...
  describe "#resbar_compile_bars" do
    def bars_for(options)
      fill, maximum, threshold, _, warning_level, danger_level, marker =
        helper.send(:resbar_prepare_options, options)
      return(
        helper.send(
          :resbar_compile_bars,
          fill,
    ...
    it "renders a single plain bar for fill < maximum and no threshold" do
      [
        ...
        expect(
          bars_for(fill: fill, maximum: 100, warning_level: 0.8),
        ).to contain_exactly(
    ...
    it "marks the empty area beyond the threshold for threshold < maximum" do
      expect(
        bars_for(fill: 25, maximum: 100, threshold: 100),
      ).to contain_exactly({ type: "default", percent: 25, label: "25" })
      expect(
        bars_for(fill: 25, maximum: 100, threshold: 60),
      ).to contain_exactly(
      ...
      expect(
        bars_for(fill: 60, maximum: 100, threshold: 60),
      ).to contain_exactly(
    ...
    it "marks the filled area beyond the threshold" do
      expect(
        bars_for(fill: 85, maximum: 100, threshold: 60),
      ).to contain_exactly(
      ...
      expect(
        bars_for(fill: 100, maximum: 100, threshold: 60),
      ).to contain_exactly(
    ...
    it "uses the threshold as upper bound when maximum < 0" do
      ...
      expect(
        bars_for(fill: 200, maximum: -1, threshold: 50),
      ).to contain_exactly(
