...
module SharedFilesystemStorage
  class Pool < Core::ServiceLayer::Model
    ...
    def total_capacity
      Core::DataType.new(:bytes, :giga).format(capabilities['total_capacity_gb'])
    end

    def allocated_capacity
      Core::DataType.new(:bytes, :giga).format(capabilities['allocated_capacity_gb'])
    end
    ...
    def free_capacity
      Core::DataType.new(:bytes, :giga).format(capabilities['free_capacity_gb'] - reserved_capacity_float)
    end
    ...
    def reserved_capacity_float
      (capabilities['reserved_percentage'] * capabilities['total_capacity_gb']) / 100
    end
  end
end
