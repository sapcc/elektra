...
const fetchShareNetworks = () =>
  function (dispatch) {
    ...
    ajaxHelper
      .get("/share-networks")
      .then((response) => dispatch(receiveShareNetworks(response.data)))
      .catch((error) => {
...
const fetchShareNetworksIfNeeded = () =>
  function (dispatch, getState) {
    if (shouldFetchShareNetworks(getState)) {
      return dispatch(fetchShareNetworks())
    }
  }
  ...
  function (dispatch, getState) {
    ...
    if (state.shares && state.shares.items) {
      for (let s of state.shares.items) {
        if (s.share_network_id === shareNetworkId) {
          networkShares.push(s)
        }
      }
    }
    ...
    if (networkShares.length > 0) {
      addNotice(
        `Please delete dependent shares(${networkShares.length}) first!`
      )
      return
    }
    confirm("Do you really want to delete this share network?")
      .then(() => {
        dispatch(requestDelete(shareNetworkId))
        ajaxHelper
          .delete(`/share-networks/${shareNetworkId}`)
          .then((response) => {
...
const submitNewShareNetworkForm = (values) => (dispatch, getState) =>
  new Promise((handleSuccess, handleErrors) => {
    ajaxHelper
      .post(`/share-networks`, { share_network: values })
      .then((response) => {
...
const submitEditShareNetworkForm = (values) => (dispatch, getState) =>
  new Promise((handleSuccess, handleErrors) => {
    ajaxHelper
      .put(`/share-networks/${values.id}`, { share_network: values })
      .then((response) => {
        if (response.data.errors) handleErrors({ errors: response.data.errors })
        else {
          dispatch(receiveShareNetwork(response.data))
...
const requestNetworksFailure = () => ({
  type: constants.REQUEST_NETWORKS_FAILURE,
})
...
const shouldFetchNetworkSubnets = function (state, networkId) {
  ...
  return (
    !subnets[networkId] ||
    (!subnets[networkId].isFetching && !subnets[networkId].requestedAt)
  )
}

const fetchNetworkSubnetsIfNeeded = (networkId) =>
  function (dispatch, getState) {
    if (shouldFetchNetworkSubnets(getState(), networkId)) {
      return dispatch(fetchNetworkSubnets(networkId))
    }
  }
// export
export {
