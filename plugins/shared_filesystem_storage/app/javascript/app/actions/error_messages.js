...
const fetchErrorMessages= (resourceId,page) =>
  function(dispatch,getState) {
    ...
    return ajaxHelper.get('/error_messages', {params: { page, resource_id: resourceId } }).then( (response) => {
      if (response.data.errors) {
        addError(React.createElement(ErrorsList, {errors: response.data.errors}))
      } else {
        dispatch(receiveErrorMessages(
          resourceId,response.data.error_messages, response.data.has_next
        ));
      }
    })
    .catch( (error) => {
