...
const requestReplicasFailure= () => ({type: constants.REQUEST_REPLICAS_FAILURE});
...
const reloadReplica= replicaId =>
  function(dispatch,getState) {
    if (!canReloadReplica(getState(),replicaId)) { return; }

    dispatch(requestReplica(replicaId));
    ajaxHelper.get(`/replicas/${replicaId}`).then((response) =>
...
const fetchReplicas= () =>
  function(dispatch) {
    ...
    return ajaxHelper.get('/replicas').then((response) => {
...
const fetchReplicasIfNeeded= () =>
  function(dispatch, getState) {
    if (shouldFetchReplicas(getState())) { return dispatch(fetchReplicas()); }
  }
;
...
const deleteReplica= replicaId =>
  function(dispatch, getState) {
    confirm('Are you sure?').then(() => {
      ...
      ajaxHelper.delete(`/replicas/${replicaId}`).then((response) => {
        if (response.data && response.data.errors) {
          addError(React.createElement(ErrorsList, {errors: response.data.errors}));
        }
        else dispatch(removeReplica(replicaId));
      }).catch(error => addError(`Could not delete replica (${error.message})`))
    }).catch(error => null)
  }
;

//################ SHARREPLICAE FORM ###################
const submitNewReplicaForm= (values) =>
  (dispatch) =>
...
const submitEditReplicaForm= (values) =>
  (dispatch) =>
    new Promise((handleSuccess,handleErrors) => {
