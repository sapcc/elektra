...
export default class ErrorMessageItem extends React.Component {
  ...
  detailsView = () =>
    <table className="table no-borders">
      <tbody>
        {["project_id","id","resource_type","resource_id","detail_id",
          "action_id","request_id","created_at","expires_at"].map((key,index) =>
          <tr key={index}>
            <th>{key}</th>
  ...
  render() {
    ...
    return (
      <React.Fragment>
        <tr>
          <td>
            <a onClick={(e) => { e.preventDefault(); this.toggleDetails()}}>
          ...
          <td><PrettyDate date={errorMessage.created_at}/></td>
        </tr>
        {this.state.showDetails &&
