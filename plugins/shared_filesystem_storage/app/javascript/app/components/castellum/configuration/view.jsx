    ...
    return (
      <React.Fragment>
        ...
        <ul>
          ...
          {config.high_threshold && <li>Shares will be extended when usage exceeds <strong>{percent(config.high_threshold.usage_percent)}</strong> {config.size_constraints.minimum_free && <React.Fragment>(or when free space is below <strong>{config.size_constraints.minimum_free} GiB</strong>)</React.Fragment>} for <strong>{duration(config.high_threshold.delay_seconds)}</strong>.</li>}
