...
export const columns = [
  ...
  { key: 'scraped', label: 'Last successful check', sortStrategy: 'numeric',
    sortKey: props => props.scraped_at || 0 },
  { key: 'checked', label: 'Last failed check', sortStrategy: 'numeric',
...
const AssetWithScrapingError = ({asset, share, handleDelete, handleForceDelete}) => {
  const {
    id: shareID, size, usage_percent: usagePercent,
    scraped_at: scrapedAt, checked } = asset;
    ...
    <React.Fragment>
      <tr>
        ...
        <td className='col-md-4'>
          ...
          {scrapedAt &&
            <div className='small text-muted'>Reported {usagePercent}&nbsp;% usage of {size}&nbsp;GiB</div>}
        </td>
