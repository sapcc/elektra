...
export default class ShareActions extends React.Component {
  render() {
    if (!policy.isAllowed("shared_filesystem_storage:share_delete") &&
        !policy.isAllowed("shared_filesystem_storage:share_update")) {
      return null;
    }

    const { share, parentView, isPending, handleDelete, handleForceDelete } = this.props;
    ...
    return (
      <div className='btn-group'>
        ...
        <ul className='dropdown-menu dropdown-menu-right' role="menu">
          ...
          { policy.isAllowed("shared_filesystem_storage:share_update") && !isPending &&
            <li><Link to={`/${parentView}/${share.id}/edit`}>Edit</Link></li>
          }
          ...
          { policy.isAllowed("shared_filesystem_storage:share_extend") && !isPending &&
            <li><Link to={`/${parentView}/${share.id}/edit-size`}>Extend / Shrink</Link></li>
          }

          { policy.isAllowed("shared_filesystem_storage:share_access_control") && share.status=='available' &&
            <li>
              <Link to={`/${parentView}/${share.id}/access-control`}>Access Control</Link>
            </li>
          }
          ...
          { policy.isAllowed("shared_filesystem_storage:share_reset_status") &&
            <li>
              <Link to={`/${parentView}/${share.id}/reset-status`}>Reset Status</Link>
            </li>
          }
          { policy.isAllowed("shared_filesystem_storage:share_revert_to_snapshot") &&
            <li>
              <Link to={`/${parentView}/${share.id}/revert-to-snapshot`}>Revert To Snapshot</Link>
            </li>
          }
          <li className="divider"></li>
          <li>
