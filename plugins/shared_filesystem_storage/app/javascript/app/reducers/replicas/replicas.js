...
const requestReplicasFailure=function(state){
...
const receiveReplica= function(state,{replica}) {
  ...
  // update or add
  if (index>=0) { items[index]=replica; } else { items.push(replica); }
...
export const replicas = function(state, action) {
  if (state == null) { state = initialReplicasState; }
  switch (action.type) {
    case constants.RECEIVE_REPLICAS: return receiveReplicas(state,action);
    case constants.REQUEST_REPLICAS: return requestReplicas(state,action);
    case constants.REQUEST_REPLICAS_FAILURE: return requestReplicasFailure(state,action);
    case constants.REQUEST_REPLICA: return requestReplica(state,action);
    case constants.REQUEST_REPLICA_FAILURE: return requestReplicaFailure(state,action);
    case constants.RECEIVE_REPLICA: return receiveReplica(state,action);
    case constants.REQUEST_DELETE_REPLICA: return requestDeleteReplica(state,action);
    case constants.DELETE_REPLICA_FAILURE: return deleteReplicaFailure(state,action);
    case constants.DELETE_REPLICA_SUCCESS: return deleteReplicaSuccess(state,action);
