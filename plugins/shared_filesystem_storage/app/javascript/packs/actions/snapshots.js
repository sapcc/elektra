...
const requestSnapshotsFailure= () => ({type: constants.REQUEST_SNAPSHOTS_FAILURE});
...
const canReloadSnapshot= function(state,snapshotId) {
  const { items } = state.shared_filesystem_storage.snapshots;
...
const reloadSnapshot= snapshotId =>
  function(dispatch,getState) {
    if (!canReloadSnapshot(getState(),snapshotId)) { return; }

    dispatch(requestSnapshot(snapshotId));
    ajaxHelper.get(`/snapshots/${snapshotId}`).then((response) =>
...
const shouldFetchSnapshots= function(state) {
  const { snapshots } = state.shared_filesystem_storage;
...
const submitEditSnapshotForm= (values) =>
  (dispatch) =>
    new Promise((handleSuccess,handleErrors) => {
