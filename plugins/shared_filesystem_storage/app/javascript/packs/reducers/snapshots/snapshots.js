...
export const snapshots = function(state, action) {
  if (state == null) { state = initialSnapshotsState; }
  switch (action.type) {
    ...
    case constants.REQUEST_DELETE_SNAPSHOT: return requestDeleteSnapshot(state,action);
    case constants.DELETE_SNAPSHOT_FAILURE: return deleteSnapshotFailure(state,action);
    case constants.DELETE_SNAPSHOT_SUCCESS: return deleteSnapshotSuccess(state,action);
