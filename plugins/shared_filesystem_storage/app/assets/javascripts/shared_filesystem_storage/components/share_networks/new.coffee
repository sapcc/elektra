...
{ connect } = ReactRedux
{ updateShareNetworkForm, submitShareNetworkForm, fetchNetworkSubnetsIfNeeded } = shared_filesystem_storage

NewShareNetwork = React.createClass
  ...
  componentDidMount: ->
    @loadShareNetworkSubnets(@props)

  componentWillReceiveProps: (nextProps) ->
    @loadShareNetworkSubnets(nextProps)

  loadShareNetworkSubnets:(props)->
    shareNetwork = props.shareNetworkForm.data
    if shareNetwork.neutron_net_id and !props.subnets
      props.loadSubnets(shareNetwork.neutron_net_id)
    ...
    form className: 'form form-horizontal', onSubmit: ((e) -> e.preventDefault(); handleSubmit()),
      ...
      div className: 'modal-footer',
        ...
        React.createElement ReactFormHelpers.SubmitButton,
          ...
          onSubmit: (() => handleSubmit(@props.close))
