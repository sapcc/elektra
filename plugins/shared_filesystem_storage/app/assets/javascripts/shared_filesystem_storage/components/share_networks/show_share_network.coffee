...
shared_filesystem_storage.ShowShareNetwork = React.createClass 
  ...
  open: (shareNetwork) -> 
    ...
    @props.loadSubnets(shareNetwork.neutron_net_id) unless @props.subnets[shareNetwork.neutron_net_id]   
  ...
  render: ->
    React.createElement shared_filesystem_storage.Modal, ref: 'modal', onHidden: @handleClose,
      ...
      if @state.shareNetwork
        ...
        tabs = {
          "Overview": table className: 'table no-borders',
            tbody null,
              tr null,
        ...
        if network
          tabs["Neutron Network"] = table className: 'table no-borders',
            tbody null,
              tr null,
                th style: {width: '30%'}, 'Name'
        ...
        if subnet
          tabs['Neutron Subnet'] = table className: 'table no-borders',
            tbody null,
              tr null,
                th style: {width: '30%'}, 'Name'
                td null, subnet.name
