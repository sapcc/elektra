  ...
  module CloudAdmin
    module PoolsHelper
      ...
      def accumulated_pools(pools)
        pools
          .each_with_object({}) do |pool, map|
            reserved_capacity_float =
              (
                pool.capabilities["reserved_percentage"] *
            ...
            map[pool.availability_zone][:total_capacity] += pool.capabilities[
              "total_capacity_gb"
            ]
            map[pool.availability_zone][
              :allocated_capacity
            ] += pool.capabilities["allocated_capacity_gb"]
            ...
            map[pool.availability_zone][:free_capacity] += pool.capabilities[
              "free_capacity_gb"
            ] - reserved_capacity_float
          ...
          .each do |data|
            data[:allocated_capacity_percentage] = (
              data[:allocated_capacity] / data[:total_capacity] * 100
            ).round(2)
            data[:free_capacity_percentage] = (
              data[:free_capacity] / data[:total_capacity] * 100
            ).round(2)
            data[:reserved_capacity_percentage] = (
              data[:reserved_capacity] / data[:total_capacity] * 100
            ).round(2)

            data[:total_capacity] = Core::DataType.new(:bytes, :giga).format(
              data[:total_capacity],
            )
            data[:allocated_capacity] = Core::DataType.new(
              :bytes,
              :giga,
            ).format(data[:allocated_capacity])
            data[:free_capacity] = Core::DataType.new(:bytes, :giga).format(
              data[:free_capacity],
            )
            data[:reserved_capacity] = Core::DataType.new(:bytes, :giga).format(
