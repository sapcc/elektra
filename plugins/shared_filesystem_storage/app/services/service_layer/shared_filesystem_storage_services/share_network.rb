  ...
  module SharedFilesystemStorageServices
    ...
    module ShareNetwork
      ...
      def share_networks(filter = {})
        elektron_shares.get('share-networks', filter)
                       .map_to('body.share_networks', &share_network_map)
      end

      def share_networks_detail(filter = {})
        elektron_shares.get('share-networks/detail', filter)
                       .map_to('body.share_networks', &share_network_map)
      end
      ...
      def find_share_network!(id)
        elektron_shares.get("share-networks/#{id}", filter)
                       .map_to('body.share_network', &share_network_map)
      end
      ...
      def add_security_service_to_share_network(security_service_id, id)
        elektron_shares.post("share-networks/#{id}/action") do
          {
