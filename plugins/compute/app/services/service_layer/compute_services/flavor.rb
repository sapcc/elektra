  ...
  module ComputeServices
    ...
    module Flavor
      ...
      def flavors(filter = {})
        elektron_compute
          .get(
            "flavors/detail",
            filter,
            { headers: { "X-OpenStack-Nova-API-Version" => "2.65" } },
          )
          .map_to("body.flavors", &flavor_map)
      ...
      def find_flavor!(flavor_id, use_cache = false)
        ...
        flavor_data =
          if use_cache
            Rails
              .cache
              .fetch(cache_key, expires_in: 24.hours) do
                elektron_compute.get("flavors/#{flavor_id}").body["flavor"]
              end
          else
            data = elektron_compute.get("flavors/#{flavor_id}").body["flavor"]
      ...
      def create_flavor(attributes)
        elektron_compute.post("flavors") { { "flavor" => attributes } }.body[
          "flavor"
        ]
      end
      ...
      def create_flavor_metadata(flavor_id, flavor_extras)
        elektron_compute
          .post("flavors/#{flavor_id}/os-extra_specs") do
      ...
      def delete_flavor_metadata(flavor_id, key)
        elektron_compute.delete("flavors/#{flavor_id}/os-extra_specs/#{key}")
      end
    end
  end
end
