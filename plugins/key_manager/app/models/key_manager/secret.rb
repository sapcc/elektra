  ...
  class Secret < Core::ServiceLayer::Model
    ...
    module Type
      ...
      def self.to_hash
        {
          SYMMETRIC.to_sym =>
            "Used for storing byte arrays such as keys
                               suitable for symmetric encryption",
          PUBLIC.to_sym =>
            "Used for storing the public key of an asymmetric
                            keypair",
          PRIVATE.to_sym =>
            "Used for storing the private key of an asymmetric
                             keypair",
          PASSPHRASE.to_sym => "Used for storing plain text passphrases",
          CERTIFICATE.to_sym =>
            "Used for storing cryptographic certificates
                                 such as X.509 certificates",
          OPAQUE.to_sym =>
            "Used for backwards compatibility with previous
