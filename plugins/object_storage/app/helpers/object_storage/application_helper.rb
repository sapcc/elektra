  ...
  module ApplicationHelper
    ...
    def make_breadcrumb(container_name, path = "")
      ...
      crumbs = [
        link_to(
          container_name,
          plugin("object_storage").list_objects_path(encoded_container_name),
        ),
      ]
      ...
      elements.each_with_index do |name, idx|
        link =
          plugin("object_storage").list_objects_path(
            encoded_container_name,
            path: elements[0..idx].join("/"),
          )
    ...
    def format_bytes(value_in_bytes)
      content_tag(:span, title: "#{value_in_bytes} bytes") do
        Core::DataType.new(:bytes).format(value_in_bytes)
      end
    end
  end
end
