module ObjectStorage
  class FoldersController < ObjectStorage::ApplicationController
    ...
    def create_object
      @form =
        ObjectStorage::Forms::CreateObject.new(
          params.require(:forms_create_object),
        )
      ...
      services.object_storage.create_object(
        @container_name,
        @object.path + @form.name,
        @form.file,
      )
      # to prevent problems with weird container names like "echo 1; rm -rf *)"
      # the name must be form encoded to load it properly
      back_to_object_list(
        URI.encode_www_form_component(@container_name),
        URI.encode_www_form_component(@object.path),
      )
    end
    ...
    def create_folder
      @form =
        ObjectStorage::Forms::CreateFolder.new(
          params.require(:forms_create_folder),
        )
      unless @form.validate
      ...
      services.object_storage.create_folder(
        @container_name,
        @object.path + @form.name,
      )
    ...
    def back_to_object_list(container_name, path)
      respond_to do |format|
        format.js do
          @objects =
            services.object_storage.list_objects_at_path(container_name, path)
