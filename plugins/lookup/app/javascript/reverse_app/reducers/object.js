...
const requestObjectInfo= function(state,{requestedAt}) {
  state['info'] = Object.assign(
    {}, infoState, state['info'], {isFetching: true, requestedAt, data: null, error: null}
  )
  return {...state}
};
...
const receiveObjectInfo= function(state,{data, receivedAt}) {
  state['info'] = Object.assign(
    {}, infoState, state['info'], {isFetching: false, receivedAt, data, error: null}
  )
  return {...state}
};
...
const requestObjectInfoFailure= function(state, error) {
  state['info'] = Object.assign(
    {}, infoState, state['info'], {isFetching: false,  data: null, error}
  )
  return {...state}
};
...
export const object = function(state, action) {
  ...
  switch (action.type) {
    ...
    case constants.REQUEST_OBJECT: return requestObject(state,action);
    case constants.REQUEST_OBJECT_FAILURE: return requestObjectFailure(state,action);
    case constants.RECEIVE_OBJECT: return receiveObject(state,action);


    case constants.REQUEST_OBJECTINFO: return requestObjectInfo(state,action);
    case constants.REQUEST_OBJECTINFO_FAILURE: return requestObjectInfoFailure(state,action);
    case constants.RECEIVE_OBJECTINFO: return receiveObjectInfo(state,action);

    case constants.RESET_STORE: return resetObject(state, action);
