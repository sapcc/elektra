  ...
  (dispatch,getState) =>
    new Promise((handleSuccess,handleErrors) => {
      ...
      ajaxHelper.post(
        ...
        dispatch(requestObjectFailure(`Could not load object (${error.message})`))
      })
    })
);
...
const fetchObjectInfo= (searchValue,objectId,searchBy) =>
  function(dispatch,getSate) {
    ...
    ajaxHelper.get(`/reverselookup/object_info/${objectId}?searchBy=${searchBy}`).then( (response) => {
    ...
    .catch( (error) => {
      dispatch(requestObjectInfoFailure(`Could not load object info (${error.message})`));
    });
  }
