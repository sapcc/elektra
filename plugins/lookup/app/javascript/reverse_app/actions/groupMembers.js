...
const fetchGroupMembers= (groupId) =>
  function(dispatch) {
    ...
    ajaxHelper.get(`/reverselookup/group_members/${groupId}`).then( (response) => {
      return dispatch(receiveGroupMembers(groupId, response.data));
    })
    .catch( (error) => {
      dispatch(requestGroupMembersFailure(groupId, `Could not load group members (${error.message})`));
    });
  }
