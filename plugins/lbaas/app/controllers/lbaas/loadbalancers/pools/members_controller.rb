  ...
  module Loadbalancers
    module Pools
      class MembersController < ApplicationController
        ...
        def add
          enforce_permissions('lbaas:member_create')

          ips = params['ips'] || []
          @new_members = []
          ips.keys.each do |ip|
            member = services.lbaas.new_pool_member(id: SecureRandom.hex)
            member.attributes = { pool_id: params[:pool_id], address: ip, weight: 1 }
            @new_members << member
          end
          render 'lbaas/loadbalancers/pools/members/add_members.js'
        end
        ...
        def create
          ...
          new_servers.each do |new_member|
            begin
              member = services.lbaas.new_pool_member
              ...
              msuccess = services.lbaas.execute(@loadbalancer.id) { member.save }
              unless msuccess
                success = false
        ...
        def destroy
          ...
          member_id = params[:id]
          @member = services.lbaas.find_pool_member(pool_id, member_id)
          services.lbaas.execute(@loadbalancer.id) { services.lbaas.delete_pool_member(pool_id, member_id) }
          audit_logger.info(current_user, 'has deleted', @member)
          render template: 'lbaas/loadbalancers/pools/members/destroy_item.js'
        end

        # update instance table row (ajax call)
        def update_item
          @member = services.lbaas.find_pool_member(@pool.id, member_id)
