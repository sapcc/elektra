      ...
      class L7policiesController < ApplicationController
        ...
        def index
          ...
          @l7policies = paginatable(per_page: per_page) do |pagination_options|
            services.lbaas.l7policies({loadbalancer_id: params[:loadbalancer_id], listener_id: params[:listener_id],
        ...
        def new
          @pools = services.lbaas.pools({loadbalancer_id: params[:loadbalancer_id]})
          @l7policy = services.lbaas.new_l7policy
        end
        ...
        def show
          @l7policy = services.lbaas.find_l7policy(params[:id])
        end
        ...
        def update
          # @l7policy = services.lbaas.find_l7policy(params[:id])
          @l7policy = services.lbaas.new_l7policy(l7policy_params)
        ...
        def destroy
          ...
          render template: 'lbaas/loadbalancers/listeners/l7policies/destroy_item.js'
        end
        ...
        def get_unused_predefined_policies
          @policies = Lbaas::L7policy.predefined(@listener.protocol )
          used = services.lbaas.l7policies({loadbalancer_id: @loadbalancer.id, listener_id: @listener.id})
          pre_polices = []
          @policies.each do |p|
            p[:ids].each do |id|
              found = false
