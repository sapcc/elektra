...
const fetchUserRoleAssignments = (userId) =>
  (dispatch,getState) => {
    const userRoleAssignments = getState()['role_assignments']['user_role_assignments']
    if (userRoleAssignments && userRoleAssignments[userId] &&
        userRoleAssignments[userId].isFetching) return
    dispatch(requestUserRoleAssignments(userId));
    ajaxHelper.get(`/users/${userId}/role_assignments`).then( (response) => {
