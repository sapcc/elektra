...
const receiveProjectMemberRoleAssignments = (
  projectId,
  memberType,
  memberId,
  roles
...
const fetchProjectRoleAssignments = (projectId) => (dispatch, getState) => {
  const projectRoleAssignments =
    getState()["role_assignments"]["project_role_assignments"]
  if (
    projectRoleAssignments &&
    projectRoleAssignments[projectId] &&
    projectRoleAssignments[projectId].isFetching
  )
    return
  dispatch(requestProjectRoleAssignments(projectId))
  ajaxHelper
    .get(`/projects/${projectId}/role_assignments`)
    .then((response) => {
...
const updateProjectMemberRoleAssignments =
  (projectId, memberType, memberId, roles) => (dispatch) => {
    const data = { roles }
    ...
    return new Promise((handleSuccess, handleErrors) =>
      ajaxHelper
        .put(`/projects/${projectId}/role_assignments`, data, {
          timeout: 180000,
        })
        .then((response) => {
          ...
          if (response.data.errors || response.data.error)
            ...
            dispatch(
              receiveProjectMemberRoleAssignments(
                projectId,
                memberType,
                memberId,
                response.data.roles
...
export { fetchProjectRoleAssignments, updateProjectMemberRoleAssignments }
