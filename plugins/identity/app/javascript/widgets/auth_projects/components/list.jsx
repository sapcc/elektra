...
export default class List extends React.Component {
  ...
  componentDidMount() {
    ...
    this.setState(newState, () => {
      if (this.state.showSearchInput && this.searchInput)
  ...
  UNSAFE_componentWillReceiveProps = (nextProps) => {
    if (nextProps.items && nextProps.items.length > 0) {
      this.setState({
        hierarchicalProjects: this.buildHierarchy(nextProps.items),
      })
    }
  }
  ...
  renderHierarchy = (projects) => {
    ...
    return projects.map((project, index) => {
      ...
      let children = null
      if (hasChildren) {
        children = this.renderHierarchy(project.children).filter(
          (child) => child != null
        )
      }
      ...
      if (this.state.expandedLists[project.id] || searchMode)
      ...
      return (
        <li key={index} className={className}>
          <i
            className="node-icon"
            onClick={(e) => this.toggleSubtree(project.id)}
          ></i>
          <a
