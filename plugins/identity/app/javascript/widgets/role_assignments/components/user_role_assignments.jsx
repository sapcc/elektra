...
export default class UserRoleAssignments extends React.Component {
  ...
  sortData = () => {
    ...
    for(let item of this.props.items) {
      ...
      if(!key) key = item.scope && item.scope && item.scope.project && item.scope.project.id

      assignments[key] = assignments[key] || {}
      assignments[key]['domain'] = item.scope && (item.scope.domain || (item.scope.project && item.scope.project.domain))
      ...
      if (item.scope && (item.scope.domain || (item.scope.project && item.scope.project.domain)))
        assignments[key]['scope_slug'] += (item.scope.domain || (item.scope.project && item.scope.project.domain)).name
    ...
    if(!this.state.filterString || this.state.filterString.length==0) return data;
    ...
    return data.filter((item) => {
      if(this.state.filterString && this.state.filterString.length>0){
        ...
        return `${item.scope_slug} ${item.roles.map(r => r.name).join(',')}`.match(regex)
      }
    })
  }
    ...
    return(
      <React.Fragment>
        ...
        {!this.props.isFetching && data.length>0 ?
                ...
                <tr key={index}>
                  <td>
                    ...
                    <span className='info-text'>
                      {(item.domain || (item.project && item.project.domain) || {}).id}
                      {item.project && item.project.id && ` / ${item.project.id}`}
                    </span>
                  </td>
                  <td>
                    <Highlighter search={this.state.filterString}>
          ...
          :
          !this.props.isFetching && data.length == 0 && <p>No role assignments found</p>
        }
