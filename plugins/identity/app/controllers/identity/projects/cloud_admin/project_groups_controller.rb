    ...
    module CloudAdmin
      class ProjectGroupsController < ::DashboardController
        ...
        def create
          ...
          @group = services_ng.identity.groups(domain_id: @domain.id,name:params[:group_name]).first rescue nil unless @group
          @group = services_ng.identity.find_group(params[:group_name]) rescue nil unless @group
            ...
            params[:role_assignments].each do |group_id,new_group_role_ids|
              ...
              role_ids_to_add.each do |role_id|
                if available_role_ids.include?(role_id)
                  services_ng.identity.grant_project_group_role(@project.id, group_id, role_id) #rescue nil
                end
              end
              role_ids_to_remove.each do |role_id|
                if available_role_ids.include?(role_id)
                  services_ng.identity.revoke_project_group_role(@project.id, group_id, role_id) #rescue nil
                end
              end
            end
          end
        ...
        def load_role_assignments(project_id)
          @role_assignments ||= services_ng.identity.role_assignments("scope.project.id"=>project_id, include_names: true, include_subtree: true)
