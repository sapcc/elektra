...
describe Identity::ProjectsController, type: :controller do
  ...
  describe "GET show" do
    ...
    before :each do
      ...
      allow_any_instance_of(::Core::ServiceLayer::ServicesManager).to receive(:available?).with(:cost_control).and_return(true)
      allow_any_instance_of(::Core::ServiceLayer::ServicesManager).to receive(:available?).with(:networking).and_return(true)
      allow_any_instance_of(::Core::ServiceLayer::ServicesManager).to receive(:available?).with(:resource_management).and_return(true)
    end
  ...
  describe "GET show_wizard" do
    ...
    context "cost_control is not finished" do
      before :each do
        allow(controller).to receive(:update_cost_control_wizard_status).and_return(false)
      end
    ...
    context "networking is not finished" do
      before :each do
        allow(controller).to receive(:update_networking_wizard_status).and_return(false)
      end
    ...
    context "cost_control service is not available" do
      before :each do
        allow_any_instance_of(::Core::ServiceLayer::ServicesManager).to receive(:available?).with(:cost_control).and_return(false)
        allow_any_instance_of(::Core::ServiceLayer::ServicesManager).to receive(:available?).with(:networking).and_return(true)
        allow_any_instance_of(::Core::ServiceLayer::ServicesManager).to receive(:available?).with(:resource_management).and_return(true)
      end
    ...
    context "cost_control and networking services are not available" do
      before :each do
        allow_any_instance_of(::Core::ServiceLayer::ServicesManager).to receive(:available?).with(:cost_control).and_return(false)
        allow_any_instance_of(::Core::ServiceLayer::ServicesManager).to receive(:available?).with(:networking).and_return(false)
        allow_any_instance_of(::Core::ServiceLayer::ServicesManager).to receive(:available?).with(:resource_management).and_return(true)
      end
    ...
    context "no service is available" do
      before :each do
        allow_any_instance_of(::Core::ServiceLayer::ServicesManager).to receive(:available?).with(:cost_control).and_return(false)
        allow_any_instance_of(::Core::ServiceLayer::ServicesManager).to receive(:available?).with(:networking).and_return(false)
        allow_any_instance_of(::Core::ServiceLayer::ServicesManager).to receive(:available?).with(:resource_management).and_return(false)
      end
