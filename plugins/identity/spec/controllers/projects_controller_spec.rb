  ...
  describe "GET show" do
    ...
    before :each do
      ...
      allow_any_instance_of(::Core::ServiceLayer::ServicesManager).to receive(
        :available?,
      ).with(:masterdata_cockpit).and_return(true)
      allow_any_instance_of(::Core::ServiceLayer::ServicesManager).to receive(
        :available?,
  ...
  describe "GET show_wizard" do
    ...
    before :each do
      ...
      allow(controller).to receive(
        :update_masterdata_cockpit_wizard_status,
      ).and_return(true)
      allow(controller).to receive(:update_sharding_wizard_status).and_return(
        true,
      )
      allow_any_instance_of(::Core::ServiceLayer::ServicesManager).to receive(
        :available?,
    ...
    context "networking is not finished" do
      before :each do
        allow(controller).to receive(
          :update_networking_wizard_status,
        ).and_return(false)
      end
