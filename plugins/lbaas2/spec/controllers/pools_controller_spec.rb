...
describe Lbaas2::Loadbalancers::PoolsController, type: :controller do
  ...
  describe "GET 'index'" do
    before :each do
      ...
      allow_any_instance_of(Lbaas2::Loadbalancers::PoolsController).to receive(
        :extend_pool_data,
      ).and_return(double("cached_pools").as_null_object)
    end
      ...
      pool =
        double(
          ...
          service:
            double(
              "octavia",
              get: double("get", map_to: double("pool", to_json: {})),
            ),
        )
      allow_any_instance_of(ServiceLayer::Lbaas2Service).to receive(
        :elektron,
