...
const requestLoadbalancers = (state) => ({
  ...state,
  isLoading: true,
  error: null,
})

const receiveLoadbalancers = (
  state,
  { loadbalancers, has_next, limit, sort_key, sort_dir }
) => {
  ...
  const marker =
    loadbalancers.length > 0 ? loadbalancers[loadbalancers.length - 1].id : null
  // sort
...
const receiveLoadbalancer = (state, { loadbalancer }) => {
  ...
  if (index >= 0) {
    items[index] = loadbalancer
  } else {
    items.push(loadbalancer)
  }
  // sort
