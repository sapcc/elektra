...
const NewMemberListItem =  ({member, index, onRemoveMember, results}) => {
  ...
  <tr>
    <td>
      ...
      {!member.saved &&
        <React.Fragment>
          <FormInput type="hidden" name={`member[${member.id}][identifier]`} value={member.id}/>
          <FormInput type="hidden" name={`member[${member.id}][index]`} value={index}/>
        </React.Fragment>
      }
    </td>
    <td>
    ...
    <td>
      {member.saved ?
        ...
        :    
        <FormInput type="number" name={`member[${member.id}][weight]`} value={member.weight || 1}/>
      }
    </td>
    <td>
      {member.saved ?
