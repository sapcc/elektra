...
const EditMember = (props) => {
  ...
  const loadMember = () => {
    console.log('fetching member to edit')
    setMember({...member, isLoading:true, error: null})
    fetchMember(loadbalancerID, poolID, memberID).then((data) => {
      setMember({...member, isLoading:false, item: data.member, error: null})
  ...
  const loadMembers = () => {
    console.log('fetching members for table')
    setMembers({...members, isLoading:true})
    fetchMembers(loadbalancerID,poolID).then((data) => {
  ...
  */
  ...
  const onSubmit = (values) => {
    setFormErrors(null)
    return updateMember(loadbalancerID, poolID, memberID, values).then((response) => {
  ...
  return ( 
    <Modal
      ...
      {member.error ?
        <Modal.Body>
          <ErrorPage headTitle="Edit Member" error={member.error} onReload={loadMember}/>
        </Modal.Body>
      :
        <React.Fragment>
          {member.isLoading ? 
                  ...
                  <Table className="table new_members" responsive>
                    ...
                    {allMembers.length>0 &&
                      allMembers.map( (member, index) =>
                        <NewMemberListItem member={member} key={member.id} index={index} results={submitResults[member.id]}/>
                      )
                    }
                    </tbody>
                  </Table>
                  { members.isLoading ? <React.Fragment><span className='spinner'/> Loading Members... </React.Fragment>: '' }
