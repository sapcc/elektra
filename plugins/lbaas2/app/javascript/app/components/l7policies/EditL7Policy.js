...
const EditL7Policy = (props) => {
  ...
  const {fetchL7Policy, actionTypes, actionRedirect, codeTypes, updateL7Policy} = useL7Policy()
  ...
  const loadL7policy = () => {
    ...
    fetchL7Policy(loadbalancerID, listenerID, l7policyID).then((data) => {
      setL7policy({...l7policy, isLoading:false, item: data.l7policy, error: null})
    })
    .catch( (error) => {      
  ...
  const loadPoolsForSelect = () => {
    ...
    fetchPoolsForSelect(loadbalancerID).then((data) => {
      setPoolsLoaded(true)
  ...
  const setSelectActionType = () => {
    const selectedOption = actionTypes().find(i=>i.value == (l7policy.item.action || "").trim());
  ...
  const setSelectRedirectPoolID = () => {
    if(l7policy.item.redirect_pool_id){
      const selectedOption = pools.items.find(i=>i.value == (l7policy.item.redirect_pool_id || "").trim());
      setRedirectPoolID(selectedOption)
    }
  }

  const setSelectRedirectCode = () => {
    const selectedOption = codeTypes().find(i=>i.value == (l7policy.item.redirect_http_code));
  ...
  */
  ...
  return ( 
    <Modal
      ...
      {l7policy.error ?
        <Modal.Body>
          <ErrorPage headTitle="Edit L7 Policy" error={l7policy.error} onReload={loadL7policy}/>
        </Modal.Body>
      :
        <React.Fragment>
          {l7policy.isLoading ? 
            ...
            <Form
              ...
              <Modal.Body>
                ...
                <Form.ElementHorizontal label='Action' name="action" required>
                  <SelectInput name="action" items={actionTypes()} onChange={onSelectAction} value={actionType}/>
                  <span className="help-block">
                    <i className="fa fa-info-circle"></i>
                ...
                {showRedirectHttpCode &&
                  <div className="advanced-options advanced-options-minus-margin">
                    ...
                    <SelectInput name="redirect_http_code" items={codeTypes()} onChange={onSelectCode} value={redirectCode}/>
                      <span className="help-block">
                        <i className="fa fa-info-circle"></i>
                ...
                {showRedirectPoolID &&
                  <div className="advanced-options">
                    <Form.ElementHorizontal label='Redirect Pool ID' name="redirect_pool_id">
                      <SelectInput name="redirect_pool_id" isLoading={pools.isLoading} items={pools.items} onChange={onSelectPoolChange} value={redirectPoolID}/>
                ...
                <Form.ElementHorizontal label='Tags' name="tags">
                  <TagsInput name="tags" initValue={l7policy.item && l7policy.item.tags}/>
                  <span className="help-block">
                    <i className="fa fa-info-circle"></i>
