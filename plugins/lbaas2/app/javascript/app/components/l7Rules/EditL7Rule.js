...
const EditL7Rule = (props) => {
  ...
  const {fetchL7Rule, ruleTypes, ruleCompareTypes, updateL7Rule, ruleTypeKeyRelation} = useL7Rule()
  ...
  const loadL7rule = () => {
    ...
    fetchL7Rule(loadbalancerID, listenerID, l7policyID, l7ruleID).then((data) => {
      setL7rule({...l7rule, isLoading:false, item: data.l7rule, error: null})
    })
    .catch( (error) => {      
  ...
  const setSelectType = () => {
    const selectedOption = ruleTypes().find(i=>i.value == (l7rule.item.type || "").trim());
    onSelectType(selectedOption)
  }

  const setSelectCompareType = () => {
    const selectedOption = ruleCompareTypes().find(i=>i.value == (l7rule.item.compare_type || "").trim());
  ...
  */
  ...
  const onSubmit = (values) => {
    ...
    return updateL7Rule(loadbalancerID, listenerID, l7policyID, l7ruleID, values).then((response) => {
      addNotice(<React.Fragment>L7 Rule <b>{response.data.l7rule.type}</b> ({response.data.l7rule.id}) is being updated.</React.Fragment>)
      // fetch the policy again containing the new l7rule
      persistL7Policy(loadbalancerID, listenerID, l7policyID).catch(error => {
      })
      close()
    }).catch(error => {
  ...
  return ( 
    <Modal
      ...
      {l7rule.error ?
        <Modal.Body>
          <ErrorPage headTitle="Edit L7 Rule" error={l7rule.error} onReload={loadL7rule}/>
        </Modal.Body>
      :
      <React.Fragment>
        {l7rule.isLoading ? 
          ...
          <Form
            ...
            <Modal.Body>
              ...
              <Form.ElementHorizontal label='Type' name="type" required>
                <SelectInput name="type" items={ruleTypes()} onChange={onSelectType} value={ruleType}/>
              ...
              <Form.ElementHorizontal label='Compare Type' name="compare_type" required>
                <SelectInput name="compare_type" items={ruleCompareTypes()} onChange={onSelectCompareType} value={ruleCompareType}/>
                <span className="help-block">
                  <i className="fa fa-info-circle"></i>
              ...
              <Form.ElementHorizontal label='Tags' name="tags">
                <TagsInput name="tags" initValue={l7rule.item && l7rule.item.tags}/>
                <span className="help-block">
                  <i className="fa fa-info-circle"></i>
