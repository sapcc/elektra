  ...
  class L7rule < Core::ServiceLayer::Model
    ...
    validates :key,
              format: {
                with: /\A[a-zA-Z!#$%&'*+-.^_`|~]+\z/,
                message:
                  "Invalid characters in value. See RFCs 2616, 2965, 6265, 7230.",
              },
              if: :type_header_cookie?
    validates :value,
              presence: true,
              format: {
                ...
                message:
                  "Invalid characters in value. See RFCs 2616, 2965, 6265.",
              }
    ...
    def persist!()
      newL7rule =
        service.create_l7rule(attributes["l7policy_id"], attributes_for_create)
    ...
    def update!()
      newL7rule =
        service.update_l7rule(
          attributes["l7policy_id"],
          id,
    ...
    def rescue_eletron_errors(e)
      ...
      e.messages.each do |m|
        ...
        keywords = {}
        m.scan(/[\w']+/) do |w|
          keywords[w.to_s] = m if attributes_for_create.key? (w)
        end
        if keywords.keys.count == 1
          # 1 key was found
          keywords.keys.each { |key| errors.add(key.to_s, keywords[key]) }
        else
