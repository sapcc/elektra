    ...
    module Pools
      class MembersController < ::AjaxController
        ...
        def index
          ...
          members =
            services.lbaas2.members(params[:pool_id], pagination_options)
          render json: {
                   members: members,
        ...
        def update
          ...
          membersParams["monitor_address"] = nil if membersParams[
            "monitor_address"
          ].blank?
          membersParams["monitor_port"] = nil if membersParams[
            "monitor_port"
          ].blank?
          newParams =
            membersParams.merge(pool_id: params[:pool_id], id: params[:id])
          member = services.lbaas2.new_member(newParams)
          if member.update
        ...
        def serversForSelect
          ...
          servers.each do |server|
            server.addresses.each do |_network_name, ip_values|
              ...
              ip_values.each do |value|
                ...
                select_servers << {
                  label: "#{value["addr"]} - #{server.name} (#{server.id})",
                  value: value["addr"],
                  address: value["addr"],
                  name: server.name,
                  id: server.id,
          ...
          render json: { servers: select_servers }
        rescue Elektron::Errors::ApiResponse => e
          render json: { errors: e.message }, status: e.code
        rescue Exception => e
          render json: { errors: e.message }, status: "500"
