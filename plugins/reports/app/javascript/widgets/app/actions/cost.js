...
const getServiceMap = function (data) {
  ...
  // convert to array
  let serviceAddedArray = Object.keys(serviceAddedHash).map(
    (i) => serviceAddedHash[i]
  )
  // sort
...
const fetchCostReport = (value) => (dispatch, getState) =>
    ...
    ajaxHelper
      ...
      .then((response) => {
        if (response.data.errors) {
          dispatch(
            requestCostReportFailure(
              `Could not load report (${response.data.errors})`
            )
          )
        } else {
      ...
      .catch((error) => {
        dispatch(
          requestCostReportFailure(`Could not load report (${error.message})`)
        )
      })
  })

export { fetchCostReport }
