...
module Resources
  class ApplicationController < DashboardController
    ...
    def fetch_hana_big_vm_data
      ...
      # { trait => flavors }
      flavors_by_numa =
        relevant_flavors.each_with_object({}) do |flavor, hash|
      ...
      # for all traits do
      custom_traits.each do |trait|
        bigvm_resource_providers =
          cloud_admin
            .resources
            .list_resource_providers({ required: "#{hana_trait},#{trait}" })
            .uniq { |rp| rp["uuid"] }
        ...
        bigvm_resource_providers.each do |rp|
          ...
          rp["az"] = host_az_map[host] if rp["shard"] || project_shards.empty?

          # ignore this resource provider unless availability_zone is present
          next unless rp["az"]

          inventories =
            cloud_admin.resources.get_resource_provider_inventory(rp["uuid"])
          ...
          result << {
            ...
            flavors:
              possible_flavors.map do |f|
                { name: f.name, disk: f.disk, vcpus: f.vcpus, ram: f.ram }
              end,
          }
        end
      end
    ...
    def fetch_big_vm_data
      ...
      bigvm_resource_providers =
        cloud_admin.resources.list_resource_providers.select do |rp|
      ...
      bigvm_resource_providers.each do |rp|
        ...
        if rp["parent_provider_uuid"].blank?
          ...
          rp_aggregates =
            cloud_admin.resources.get_resource_provider_aggregates(rp["uuid"])
          rp["parent_provider_uuid"] = rp_aggregates[
            0
          ] unless rp_aggregates.empty?
        end

        # get the host_size for current rp
        parent_inventories =
          cloud_admin.resources.get_resource_provider_inventory(
        ...
        traits =
          cloud_admin
            .resources
            .traits(rp["parent_provider_uuid"])
            .select { |t| t.starts_with? "CUSTOM_NUMASIZE_" }
            .map { |t| t.gsub("CUSTOM_NUMASIZE_", "") }
