...
const columnDefs = [
  ...
  { key: 'domain', label: 'Domain', sortStrategy: 'text',
    sortKey: props => nameOrIDOf(props.domain) },
  { key: 'project', label: 'Project', sortStrategy: 'text',
    sortKey: props => `${nameOrIDOf(props.project.domain)}/${nameOrIDOf(props.project)}` },
  { key: 'resource', label: 'Resource', sortStrategy: 'text',
    sortKey: props => `${t(props.service) || ''} ${t(props.resource) || ''}` },
  { key: 'quota', label: 'Quota', sortStrategy: 'numeric',
    sortKey: props => props.quota || 0 },
  { key: 'domain_quota', label: 'Quota', sortStrategy: 'numeric',
    sortKey: props => props.domain_quota || 0 },
  { key: 'projects_quota', label: 'Assigned to projects', sortStrategy: 'numeric',
    sortKey: props => props.projects_quota || 0 },
  { key: 'backend_quota', label: 'Backend Quota', sortStrategy: 'numeric',
...
const cellContents = {
  ...
  projects_quota: ({ projects_quota: value, unit}) => valueWithUnit(value, unit),
...
const InconsistencyRow = (props) => {
  ...
  for (const key of props.columnKeys) {
    ...
    cells.push(<td className={cssClass} key={key}>{contents}</td>);
  }
...
export default class Inconsistencies extends React.Component {
  ...
  render() {
    ...
    const columns = tableDef.columnKeys.map(key => columnDefs.find(c => c.key == key));
          ...
          <ul className="nav nav-pills nav-stacked">
            { tableDefs.map((tableDef, idx) => (
              <li key={idx} role="presentation" className={idx == this.state.active ? "active" : ""}>
                <a href="#" onClick={(e) => this.handleSelect(idx, e)}>{tableDef.label}</a>
              </li>
            ))}
          </ul>
        </div>
        <div className='col-md-10'>
          <DataTable columns={columns} pageSize={8}>
            {tableData.map((rowData, idx) =>
              <InconsistencyRow key={idx} {...rowData} columnKeys={tableDef.columnKeys} />
            )}
          </DataTable>
        </div>
      </div>
