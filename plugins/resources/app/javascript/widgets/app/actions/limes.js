...
export const fetchData = (scopeData) =>
...
export const fetchDataIfNeeded = (scopeData) =>
  function (dispatch, getState) {
...
export const listSubscopes = (scopeData, serviceType, resourceName) =>
  function (dispatch, getState) {
    const scope = new Scope(scopeData)
...
export const fetchCapacity = (scopeData) =>
  function (dispatch, getState) {
    dispatch(requestCapacity())
    const scope = new Scope(scopeData)
...
export const fetchCapacityIfNeeded = (scopeData) =>
  function (dispatch, getState) {
    const state = getState()
    if (
      state.limes.capacityData.isFetching ||
      state.limes.capacityData.requestedAt
    ) {
      return
    }
...
export const syncProject = ({ domainID, projectID }) =>
  function (dispatch, getState) {
    dispatch(syncProjectRequested())
    ajaxHelper
      .post(`/v1/domains/${domainID}/projects/${projectID}/sync`)
      .then((response) => {
...
export const pollRunningSyncProject = ({ domainID, projectID }) =>
  function (dispatch, getState) {
...
export const setProjectHasBursting = ({ domainID, projectID, hasBursting }) =>
  function (dispatch) {
    var requestBody = {
      project: { bursting: { enabled: hasBursting ? true : false } },
    }
    return new Promise((resolve, reject) =>
      ajaxHelper
        .put(`/v1/domains/${domainID}/projects/${projectID}`, requestBody)
        .then((response) => {
          dispatch(fetchData({ domainID, projectID }))
          resolve()
        })
...
export const setQuota = (scopeData, requestBody) =>
  function (dispatch) {
    const scope = new Scope(scopeData)

    return new Promise((resolve, reject) =>
      ajaxHelper
        .put(scope.urlPath(), requestBody)
        .then(resolve)
        .catch((error) => reject({ errors: limesErrorMessage(error) }))
    )
  }

export const simulateSetQuota = (scopeData, requestBody) =>
  function (dispatch) {
    const scope = new Scope(scopeData)

    return new Promise((resolve, reject) =>
      ajaxHelper
        .post(scope.urlPath() + "/simulate-put", requestBody)
        .then(resolve)
        .catch((error) => reject({ errors: limesErrorMessage(error) }))
    )
  }
      ...
      for (const subscope of data) {
        for (const srv of subscope.services) {
          ...
          for (const res of srv.resources) {
            ...
            if (res.annotations?.can_autoscale === "true" && "quota" in res) {
              isEmpty = false
              result[srv.type][res.name].push({
                id: subscope.id,
                name: subscope.name,
                category: res.category || srv.type,
              })
