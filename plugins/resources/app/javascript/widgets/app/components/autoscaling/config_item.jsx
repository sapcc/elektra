...
const renderConfigUI = ({
  ...
  if (editValue === null) {
    if (parsed.value === null) {
      ...
      const extraText =
        parsed.value === 0 ? `(but at least ${unit.format(1)} free)` : ""
      return (
        <span>
          Target <strong>{parsed.value}%</strong> free quota {extraText}
        </span>
      )
    }
  } else {
...
export const AutoscalingConfigItem = ({
  project,
  config,
  assetType,
  ...editorProps
    ...
    <td className="col-md-8">
      {config.isFetching ? (
        ...
        <div className="alert alert-error">
          Could not load autoscaling configuration for project
        </div>
      ) : (
        renderConfigUI({
