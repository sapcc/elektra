...
export default class DetailsResource extends React.Component {
  ...
  submit() {
    ...
    if (parsedValue.error) {
      ...
      this.props.handleAPIErrors(
        scope.formatInputError(parsedValue.error, unitName)
      )
      return
    }
    ...
    this.props
      .setQuota(this.props.metadata.id, parsedValue)
  ...
  render() {
    ...
    return (
      <tr>
        ...
        {scope.isDomain() && (
          <td className="col-md-2">{valueWithUnit(projectsQuota, unit)}</td>
        )}
        <td className={scope.isDomain() ? "col-md-1" : "col-md-2"}>
          {valueWithUnit(usage, unit)}
        </td>
        <td className={scope.isDomain() ? "col-md-1" : "col-md-2"}>
          {valueWithUnit(burstUsage || 0, unit)}
        </td>
        <td className="col-md-3">
          {isEditing ? (
            <>
              ...
              >
                {buttonCaption("Save", isSubmitting)}
              </a>{" "}
              <a
                key="cancel"
