...
export default class InitProjectModal extends React.Component {
    ...
    for (const categoryName in WIZARD_RESOURCES) {
      ...
      for (const resource of category.resources) {
        ...
        if (categoryConf.resources[resource.name]) {
          ...
          resource.scales_with &&
          categoryConf.resources[resource.scales_with.resource_name]
        ) {
  ...
  checkAvailabilityOnce(props) {
    ...
    const isAvailable = {}
    for (const categoryName in WIZARD_RESOURCES) {
      isSelected[categoryName] = WIZARD_RESOURCES[categoryName].preselect
        ? true
        : false
  ...
  render() {
    ...
    if (isChecking) {
      ...
      return (
        <div className="modal-body">
          <p>
            <span className="spinner" /> Checking package availability...
          </p>
        </div>
      )
    }
    ...
    for (const area of Object.keys(this.props.overview.areas)) {
      ...
      for (const serviceType of serviceTypes) {
        for (const categoryName of this.props.overview.categories[
          serviceType
        ]) {
    ...
    const byAreaThenByName = (categoryNameA, categoryNameB) => {
      ...
      if (infoA.area != infoB.area) return byUIString(infoA.area, infoB.area)
    ...
    return (
      ...
      <React.Fragment>
        <div className="modal-body resources">
          ...
          <p>
            Please select an initial allotment of resources for your project.
            Note that:
          </p>
          <ul>
            <li>
              Quota assignments incur costs according to our{" "}
              <a href={`${docsUrl}docs/start-pricing`}>price list</a>.
            </li>
            <li>
              You can always change your quotas later on. Large quota requests
              may be{" "}
              <a href={`${docsUrl}docs/quota/#auto-approval`}>
                subject to approval
              </a>
              .
            </li>
          ...
          <div id="package-selection">
            {Object.keys(resourceValues)
              .sort(byAreaThenByName)
              .map((categoryName) =>
                this.renderPackage(categoryName, resourceValues[categoryName])
              )}
          </div>
        </div>
        <div className="buttons modal-footer">
          ...
          >
            {buttonCaption("Submit", isSubmitting)}
          </div>
          <div
  ...
  renderPackage(categoryName, resources) {
    ...
    for (let resourceName of resourceNames) {
      ...
      const typeText =
        resource.quotaText == "1"
          ? t(resourceName + "_single")
          : t(resourceName)

      if (resource.isHighlighted) {
        highlightedResources.push(
          <div key={resourceName}>
            {resource.quotaText} {typeText}
          </div>
        )
      } else {
        mutedResources.push(
    ...
    const box = (
      ...
      >
        <h3>
          <i
            className={
              isSelected ? "fa fa-fw fa-check-square" : "fa fa-fw fa-square-o"
            }
          />
          {" " + t(categoryName)}
        </h3>
        <div className="highlighted-resources">{highlightedResources}</div>
        {mutedResources.length > 0 && (
          <div className="muted-resources">{mutedResources}</div>
        )}
      </div>
    )
