...
export default class EditModal extends React.Component {
  ...
  initializeValuesIfNecessary = (props) => {
    ...
    for (let res of resourcesWithQuota) {
      ...
      if (
        res.scales_with &&
        res.scales_with.service_type == props.category.serviceType
      ) {
        if (hasResource[res.scales_with.resource_name]) {
          inputs[res.name].isFollowing = true
        }
      }
      //disable editing for non-HQD resources
      if (res.quota_distribution_model !== "hierarchical") {
        inputs[res.name].readonlyReason = "qd"
      }
    }
  ...
  parseInputs = (additionalStateChanger = null) => {
    ...
    for (let res of this.props.category.resources.filter(tracksQuota)) {
      ...
      const input = {
        text: oldInput.text,
        isFollowing: oldInput.isFollowing,
        readonlyReason: oldInput.readonlyReason,
      }
    ...
    for (let res of this.props.category.resources.filter(tracksQuota)) {
      ...
      const baseRes = this.props.category.resources.find(
        (res) => res.name == baseResName
      )
  ...
  handleCheck = () => {
    ...
    this.props
      .simulateSetQuota(this.props.scopeData, requestBody)
  ...
  handleCheckResponse = (response) => {
    let { success, unacceptable_resources: unacceptableResources } =
      response.data
    if (success) {
      ...
      switch (error.status) {
        case 403:
          if (error.max_acceptable_quota) {
            const limit = resUnitByName[error.resource_name].format(
              error.max_acceptable_quota
            )
            if (this.props.isForeignScope) {
              input.checkResult = {
                unacceptable: `Raising beyond ${limit} not allowed`,
              }
            } else {
              input.checkResult = {
                requestRequired: `Raising beyond ${limit} requires approval`,
              }
            }
          } else {
            if (this.props.isForeignScope) {
        ...
        case 409:
          if (
            error.max_acceptable_quota != null &&
            !this.props.isForeignScope
          ) {
            ...
            input.checkResult = {
              requestRequired: `Requires approval by domain admin`,
            }
          } else {
  ...
  render() {
    ...
    for (let res of category.resources.filter(tracksQuota)) {
      ...
      if (input.readonlyReason === "qd") {
        showQDExplainer = true
      }
        ...
        <Modal.Body>
          {showQDExplainer && (
            <div className="bs-callout bs-callout-warning bs-callout-emphasize">
              <span className="cqd-explanation">
                Some or all resources in this category operate under an
                automatic quota distribution model. For those resources, quota
          ...
          {this.state.inputs &&
            sortByLogicalOrderAndName(
              category.resources.filter(tracksQuota)
            ).map((res) => (
              <Resource
                key={res.name}
                resource={res}
