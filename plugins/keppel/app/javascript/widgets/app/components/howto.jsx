...
const apiStyleOptions = [
  ...
  { value: 'domainremap', label: 'Domain-remapped style (account name in domain)' },
];
    ...
    return (
      <div className='plugin-help visible'>
        <div className='bs-callout bs-callout-info bs-callout-emphasize'>
          <a className='close-button' href='#' onClick={e => { e.preventDefault(); handleClose(); }}>x</a>
          <h4>How to use this {repoName == '<repo>' ? 'account' : 'repository'} with Docker</h4>
          <p>{makeSelectBox({
            options: apiStyleOptions,
            value: this.state.apiStyle,
            isEditable: true,
            onChange: e => { e.preventDefault(); this.setAPIStyle(e.target.value); },
          })}</p>
          <ol className='howto'>
            <li>
            ...
            <li>
              ...
              <pre><code>{`$ docker push ${fullRegistryDomain}/${fullRepoName}:<tag>`}</code></pre>
            </li>
            <li>
              To pull an image, use this command:
              <pre><code>{`$ docker pull ${fullRegistryDomain}/${fullRepoName}:<tag>`}</code></pre>
