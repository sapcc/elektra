...
const timeUnitStrings = { 's': 'seconds', 'm': 'minutes', 'h': 'hours', 'd': 'days', 'w': 'weeks', 'y': 'years' };
          ...
          <td colSpan='2' className='gcpolicy'>
            ...
            {tc.on && (
              <React.Fragment>
                ...
                {tc.older_than && `is older than ${tc.older_than.value} ${timeUnitStrings[tc.older_than.unit] || tc.older_than.unit}`}
                {tc.newer_than && `is newer than ${tc.newer_than.value} ${timeUnitStrings[tc.newer_than.unit] || tc.newer_than.unit}`}
                {tc.oldest && `is among the oldest ${tc.oldest} in this repository`}
                {tc.newest && `is among the newest ${tc.newest} in this repository`}
              </React.Fragment>
            )}
          </td>
        </tr>
  ...
  return (
    <React.Fragment>
      <tr>
        ...
        <td colSpan='2' className='gcpolicy-folder' onClick={() => setOpen(!isOpen)}>
