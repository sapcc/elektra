        ...
        <span className='hover-copier-actions'>
          {actions.map(action => (
            (copiedLabel == action.label) ? (
              <button key={action.label} className='btn btn-link btn-xs' disabled={true}>
                <i className="fa fa-copy fa-fw"></i> Copied!
              </button>
            ) : (
              <Clipboard key={action.label} className='btn btn-link btn-xs' data-clipboard-text={action.value} onSuccess={() => setCopiedLabel(action.label)}>
