...
const fetchSnapshot = (id) => (dispatch) => {
...
const deleteSnapshot = (id) => (dispatch) =>
  confirm(`Do you really want to delete the snapshot ${id}?`)
    .then(() => {
      return ajaxHelper
        .delete(`/snapshots/${id}`)
...
const fetchSnapshots = () =>
  function (dispatch, getState) {
    ...
    return ajaxHelper
      ...
      .then((response) => {
        dispatch(
          receiveSnapshots(response.data.snapshots, response.data.has_next)
        )
      })
      .catch((error) => {
...
const submitEditSnapshotForm = (id, values) => (dispatch) =>
  new Promise((handleSuccess, handleErrors) =>
    ajaxHelper
      .put(`/snapshots/${id}`, { snapshot: values })
      .then((response) => {
        dispatch(receiveSnapshot(response.data))
        handleSuccess()
      })
...
const submitResetSnapshotStatusForm = (id, values) => (dispatch) =>
  new Promise((handleSuccess, handleErrors) =>
    ajaxHelper
      .put(`/snapshots/${id}/reset-status`, values)
      .then((response) => {
        dispatch(receiveSnapshot(response.data))
        handleSuccess()
      })
