...
export default class List extends React.Component {
  ...
  filterItems = () => {
    ...
    return items.filter((i) =>
      `${i.id} ${i.name} ${i.description} ${i.volume_id} ${i.size} ${i.status}`.search(regex) >= 0
    )
  }
        ...
        {this.props.snapshots.items.length>5 &&
          <div className='toolbar'>
            <SearchField
              ...
              placeholder='name, ID, format or status' text='Searches by name, ID, format or status in visible snapshots list only.
        ...
        <table className='table snapshots'>
          ...
          <tbody>
            { items && items.length>0 ?
              ...
              :
              <tr>
                <td colSpan="6">{ isFetching ? <span className='spinner'/> : 'No snapshots found.' }</td>
              </tr>
            }
          </tbody>
        </table>
