...
export default class Volume extends React.Component {
  ...
  startPolling = () => {
    ...
    this.polling = setInterval(() =>
      this.props.reloadVolume(this.props.volume.id), 5000
    )
  }
  ...
  handleDetach = (e) => {
    e.preventDefault()
    if (!this.props.volume.attachments || this.props.volume.attachments.length==0) {
      return
    }

    this.props.detachVolume(this.props.volume.id, this.props.volume.attachments[0].attachment_id)
  }
  ...
  render() {
    ...
    return(
      <tr className={`state-${volume.status}`}>
        ...
        <td>
          {volume && volume.attachments && volume.attachments.length>0 &&
            volume.attachments.map((attachment,index) =>
              <div key={index}>
                <a href={`/${scope.domain}/${scope.project}/compute/instances/${attachment.server_id}`} data-modal={true}>{attachment.server_name || attachment.server_id}</a>
        ...
        <td className='snug'>
          { (policy.isAllowed("block_storage:volume_delete", {target: {scoped_domain_name: scope.domain}}) || policy.isAllowed("block_storage:volume_update", {target: {scoped_domain_name: scope.domain}})) &&
            <div className='btn-group'>
              ...
              <ul className="dropdown-menu dropdown-menu-right" role="menu">
                { policy.isAllowed("block_storage:volume_update", {target: {scoped_domain_name: scope.domain}}) &&
                  <li><Link to={`/volumes/${volume.id}/edit`}>Edit</Link></li>
                }
                ...
                { volume.attachments && volume.attachments.length==0 && policy.isAllowed("compute:attach_volume", {target: {scoped_domain_name: scope.domain}}) ?
                  <React.Fragment>
                    ...
                    <li><Link to={`/volumes/${volume.id}/attachments/new`}>Attach</Link></li>
                  </React.Fragment>
                  :
                  policy.isAllowed("compute:detach_volume", {target: {scoped_domain_name: scope.domain}}) &&
                    <React.Fragment>
                      <li className="divider"></li>
                      <li><a href='#' onClick={this.handleDetach}>Detach</a></li>
                    </React.Fragment>
                }
                { policy.isAllowed("block_storage:volume_delete", {target: {scoped_domain_name: scope.domain}}) && volume.status != 'in-use' &&
                  <React.Fragment>
                    <li className="divider"></li>
