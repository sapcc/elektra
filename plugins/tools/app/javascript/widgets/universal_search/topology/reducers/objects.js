...
const removeLeaves=(state,{objectId})=> {
  if(!state[objectId] || !state[objectId].children ||  state[objectId].children.length == 0)
    return state

  const childrenToBeRemoved = state[objectId].children.filter(child => {
    return child && state[child] && state[child].children && state[child].children.length == 0
  })

  const newState = {...state}

  newState[objectId] = Object.assign({}, initialObjectState, state[objectId], {
    children: (newState[objectId].children || []).filter(c => childrenToBeRemoved.indexOf(c)<0),
    requestedAt: null,
    receivedAt: null
  })
...
export default (state, action) => {
  ...
  switch (action.type) {
    ...
    case constants.REMOVE_LEAVES: return removeLeaves(state,action);
