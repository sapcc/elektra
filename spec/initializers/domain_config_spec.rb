  ...
  'domains' => [
    {
      'name' => 'all iaas',
      'regex' => '^iaas-.*$',
      'disabled_plugins' => %w[plugin1 plugin2],
      'floating_ip_networks' => ['FloatingIP-external-%DOMAIN_NAME%-network'],
      'disabled_features' => ['documentation'],
  ...
  describe 'test config' do
    ...
    describe '#feature_hidden?' do
      it 'returns true if the feature is disabled' do
        expect(DomainConfig.new('iaas-domain1').feature_hidden?('documentation')).to be true
      end

      it 'returns false if the feature is not disabled' do
        expect(DomainConfig.new('iaas-test1').feature_hidden?('documentation')).to be false
      end
    end
  end
end
