...
describe OpenstackServiceProvider::BaseObject do
  before :each do
    @driver = double('driver').as_null_object
    @base_object = OpenstackServiceProvider::BaseObject.new(@driver)
  end
  ...
  describe '::initialize' do
    it 'executes after_initialize callback' do
      expect_any_instance_of(OpenstackServiceProvider::BaseObject).to receive(:after_initialize)
      OpenstackServiceProvider::BaseObject.new(@driver)
    end
        
    context 'no params given' do
      it 'creates a new empty base object' do
        o = OpenstackServiceProvider::BaseObject.new(@driver)
    ...
    context 'params given' do
      it 'creates a new base object' do
        o = OpenstackServiceProvider::BaseObject.new(@driver, {'a' => 'test'})
        expect(o.attributes).to eq({'a'=>'test'})
      end
      
      it 'sets id from params' do
        o = OpenstackServiceProvider::BaseObject.new(@driver, {'id'=>1,'a' => 'test'})
        expect(o.id).to eq(1)
      end
    end
  end
  ...
  describe '::requires' do
    ...
    it "no error raised" do
      o = OpenstackServiceProvider::BaseObject.new(@driver, {'id'=>1,'attr1' => 'test'})
      expect{
        o.requires(:id,:attr1)
      }.not_to raise_error
    end
  ...
  describe 'save' do
    ...
    context 'id is not nil' do
      before :each do
        @o = OpenstackServiceProvider::BaseObject.new(@driver, {'id'=>1})
      end
  ...
  describe 'attribute_to_object' do
    it 'creates an object from attributes' do
      ...
      subobject = @base_object.attribute_to_object(:subobject,OpenstackServiceProvider::BaseObject)
      expect(subobject).to be_kind_of(OpenstackServiceProvider::BaseObject)
