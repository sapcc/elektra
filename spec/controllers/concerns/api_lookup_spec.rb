  ...
  describe 'api_search' do
    before :each do
      stub_const(
        ...
        'test_object' => [
          'test_service', 'find(":term")', 'all(name: ":term", param: true)'
        ]
      )
    end
    ...
    context 'service exists' do
      before :each do
        ...
        allow(service_manager).to receive(:respond_to?).with('test_service').and_return(true)
        allow(service_manager).to receive(:send).with('test_service').and_return(@service)
      end
    ...
    context 'service does not exist' do
      before :each do
        allow(service_manager).to receive(:respond_to?).with('test_service').and_return(false)
      end

      it 'raises service not found error' do
        expect do
          dummy.api_search(service_manager, 'test_object', 'test')
        end.to raise_error StandardError, 'Service test_service could not be found.'
      end
    end
  end
