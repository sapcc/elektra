...
describe ScopeController, type: :controller do
  ...
  before(:all) do
    @domain_friendly_id = FriendlyIdEntry.find_or_create_entry(
      'Domain', nil, default_params[:domain_id], 'default 1'
    )
    @project_friendly_id = FriendlyIdEntry.find_or_create_entry(
  ...
  context 'domain_id is provided, project_id is not provided' do
    describe 'GET index' do
      ...
      it 'redirects to the same action with friendly ids for domain' do
        get :index, params: {
          domain_id: AuthenticationStub.domain_id, project_id: nil
        }
      ...
      it 'redirects to the same action with friendly ids for domain and other parameters' do
        get :index, params: {
          domain_id: AuthenticationStub.domain_id, name: 'test'
        }
        expect(response).to redirect_to("/#{@domain_friendly_id.slug}/scope?name=test")
      end
    end
  end
