...
describe Core::ScopeController, type: :controller do
  controller do
    def index
      head :ok
    end
  ...
  before(:all) do
    @domain_friendly_id = Core::FriendlyIdEntry.find_or_create_entry('Domain',nil,default_params[:domain_id],'default 1')
    @project_friendly_id = Core::FriendlyIdEntry.find_or_create_entry('Project',default_params[:domain_id],default_params[:project_id],'project 1')
  end
  ...
  context "domain_id is provided, project_id is not provided" do
    ...
    describe "GET 'index'" do
      ...
      it "redirects to the same action with friendly ids for domain" do
        ...
        expect(response).to redirect_to(controller: 'core/scope', action: 'index', domain_id: @domain_friendly_id.slug)
      end
    end
  end
  ...
  context "domain_id and project_id are provided" do
    ...
    describe "GET 'index'" do
      ...
      it "redirects to the same action with friendly ids for domain and project" do
        ...
        expect(response).to redirect_to(controller: "core/scope", action: "index", domain_id: @domain_friendly_id.slug, project_id: @project_friendly_id.slug)
      end
    end
  end
