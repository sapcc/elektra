...
RSpec.describe Openstack::AdminIdentityService do
  ...
  before :each do
    ...
    allow_any_instance_of(Openstack::AdminIdentityService).to receive(:get_driver).and_return(@driver)
    @admin_identity = Openstack::AdminIdentityService.new('http://localhost:5000/v3','europe',@current_user)
  end

  describe 'domain_friendly_id' do
    before :each do
      Core::FriendlyIdEntry.delete_all
    end
      
    context 'entry exists' do
      let!(:key) {'1234-5678'}
      let!(:entry) { Core::FriendlyIdEntry.create(class_name:'Domain',key: key, name: 'Test Domain') }
    ...
    context 'entry does not exists' do
      it "should create a new entry by id" do
        ...
        }.to change{Core::FriendlyIdEntry.count}.by(1)
      end
  ...
  describe 'project_friendly_id' do
    ...
    context 'entry exists' do
      ...
      let!(:key) {'1234-5678'}
      let!(:entry) { Core::FriendlyIdEntry.create(class_name:'Project', scope: domain_id, key: key, name: 'Project 1') }
