...
module Openstack
  class AdminIdentityService < OpenstackServiceProvider::Service
    ...
    def reset_domain_friendly_id(domain_id)
      ...
      entry = Core::FriendlyIdEntry.where(sql).first
    ...
    def domain_friendly_id(domain_fid_id_or_key)
      ...
      entry = Core::FriendlyIdEntry.find_by_class_scope_and_key_or_slug('Domain',nil,domain_fid_id_or_key)
      ...
      unless entry
        ...
        if domain_attrs
          entry = Core::FriendlyIdEntry.create(class_name: 'Domain', key: domain_attrs["id"], name: domain_attrs["name"])
        end
      end
      entry
    end
    ...
    def project_friendly_id(domain_id, project_fid_or_key)
      # try to find an entry by given fid or key
      entry = Core::FriendlyIdEntry.find_by_class_scope_and_key_or_slug('Project',domain_id,project_fid_or_key)
      ...
      # create friendly_id entry
      if project_attrs
        entry = Core::FriendlyIdEntry.create(class_name: 'Project', scope: project_attrs["domain_id"], key: project_attrs["id"], name: project_attrs["name"])
      end
      entry
    end
    ##########################################################################
