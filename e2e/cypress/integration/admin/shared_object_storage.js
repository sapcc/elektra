describe("shared object storage", () => {
  ...
  it("open object storage and check access control and check ACLs button", () => {
    cy.visit(
      `/${Cypress.env(
        "TEST_DOMAIN"
      )}/admin/object-storage/containers?overlay=elektra-test%2Fshow_access_control`
    )
    cy.contains("Read ACLs")
    cy.contains("a", "Check ACLs").click()
    cy.contains("No Read ACLs found")
  })
  ...
  it("open object storage and check elektra-test container", () => {
    cy.visit(
      `/${Cypress.env(
        "TEST_DOMAIN"
      )}/admin/object-storage/containers/elektra-test`
    )
  ...
  it("open object storage and elektra-test container and check actions", () => {
    ...
    cy.contains(
      "This dialog only accepts files smaller than 1 MiB. To upload larger files, please use a different client."
    )
    cy.contains("button", "Cancel").click()
  })
  ...
  it("open elektra-test container directly and check item", () => {
    cy.visit(
      `/${Cypress.env(
        "TEST_DOMAIN"
      )}/admin/object-storage/containers/elektra-test/list/this+is+a+test%252F`
    )
    cy.contains("[data-test=page-title]", "Object Storage")
    cy.contains("a", "docker.jpg")
    cy.get("div.btn-group").click()
    cy.contains("a", "Properties").click()
    cy.contains("File: this is a test/docker.jpg")
    cy.contains("button", "Cancel").click()

    cy.get("div.btn-group").click()
    cy.contains("a", "Copy").click()
    cy.contains("Copy object: this is a test/docker.jpg")
    cy.contains("button", "Cancel").click()

    cy.get("div.btn-group").click()
    cy.contains("a", "Move/Rename").click()
    cy.contains("Move and/or rename object: this is a test/docker.jpg")
    cy.contains("button", "Cancel").click()
  })
})
