module ApplicationHelper
  ...
  def page_title
    "CCloud #{@scoped_domain_name} #{current_region if respond_to?(:current_region) and current_region}"
  end
  ...
  def plugin_available?(name)
    # domain_config is created in the concern (app/controllers/concerns/scope_handler.rb)
    # and called in the dashboard controller (app/controllers/dashboard_controller.rb)
    # This mehtod is used to check if a plugin is available for the current domain when
    # rendering the navigations.
    # The server side part is done in the scope_controller
    # check if @domain_config is nil, this is the case for controllers which do not
    # include the scope_handler concern.
    return false if !@domain_config.nil? && @domain_config.plugin_hidden?(name.to_s)

    respond_to?("#{name}_plugin".to_sym)
  end
  ...
  # ---------------------------------------------------------------------------------------------------
  def current_project_parents(&block)
    current_project =
      ObjectCache.where(
  ...
  def active_project_tree(active_project, auth_projects, options = {})
    ...
    parent_ids.compact.each { |key| tree = { key => tree } } unless parent_ids.blank?
    ...
    content_tag(:ul, class: options.delete(:class)) do
      if subprojects.is_a?(Array)
        ...
        subprojects.each do |k, v|
          ...
          next unless project or v.is_a?(Hash)
          ...
          result << content_tag(
            ...
            capture do
              ...
              concat subprojects_tree(v, auth_projects, options) if v.is_a?(Hash)
            end
          end
        end
        result.join("\n").html_safe
  ...
  def parents_tree(parents_project_ids, auth_projects, options = {})
    return if parents_project_ids.blank?

    parents_project_ids = parents_project_ids.compact
    unless auth_projects.is_a?(Hash)
      auth_projects =
        auth_projects.each_with_object({}) do |pr, hash|
          hash[pr.id] = pr
        end
    end

    return unless parents_project_ids and parents_project_ids.length > 0
  ...
  def active_service_breadcrumb
    ...
    crumb = 'Home' # Default case, only visible on domain home page
    if active_service.blank?
      # no service selected, if project is available this is the project home page -> print project name
      crumb = 'Project Overview' unless @active_project.blank?
    else
