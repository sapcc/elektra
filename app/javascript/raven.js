        ...
        function (_dereq_, module, exports) {
          ...
          Raven.prototype = {
            ...
            config: function (dsn, options) {
              ...
              this._globalOptions.ignoreErrors.push(
                /^Javascript error: Script error\.? on line 0$/
              )

              // join regexp rules into one big rule
              this._globalOptions.ignoreErrors = joinRegExp(
                this._globalOptions.ignoreErrors
              )
              this._globalOptions.ignoreUrls = this._globalOptions.ignoreUrls
                .length
                ? joinRegExp(this._globalOptions.ignoreUrls)
                : false
              this._globalOptions.whitelistUrls = this._globalOptions
                .whitelistUrls.length
                ? joinRegExp(this._globalOptions.whitelistUrls)
                : false
              this._globalOptions.includePaths = joinRegExp(
                this._globalOptions.includePaths
              )
              this._globalOptions.maxBreadcrumbs = Math.max(
                0,
                Math.min(this._globalOptions.maxBreadcrumbs || 100, 100)
              ...
              TraceKit.collectWindowErrors =
                !!this._globalOptions.collectWindowErrors
                    ...
                    function (orig) {
                      return function (evt, fn, capture, secure) {
                        try {
                          fn =
                            fn &&
                            (fn.__raven_wrapper__ ? fn.__raven_wrapper__ : fn)
                        } catch (e) {
            ...
            _instrumentBreadcrumbs: function () {
              ...
              var hasPushState =
                !isChromePackagedApp && window.history && history.pushState
            ...
            _makeRequest: function (opts) {
              ...
              function handler() {
                if (request.status === 200) {
                  ...
                  opts.onError(
                    new Error("Sentry error code: " + request.status)
                  )
                }
              }
