  ...
  navigation.items do |primary|
   ...
   primary.item :hana, 'Bare Metal Data Processing & HANA', nil, html: {class: "fancy-nav-header", 'data-icon': "hana-icon" }, if: -> {services.available?(:bare_metal_hana,:nodes) } do |bare_metal_hana_nav|
    ...
    primary.item :access_management, 'Users & Authorizations', nil,
      ...
      if: -> {services.available?(:identity) and (current_user.is_allowed?('identity:project_member_list') or current_user.is_allowed?('identity:project_group_list')) } do |access_management_nav|
