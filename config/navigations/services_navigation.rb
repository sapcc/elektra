  ...
  primary.item :compute, 'Compute', nil, html: {class: "fancy-nav-header", 'data-icon': "compute-icon"},
   ...
   primary.item :hana, 'Bare Metal Data Processing & HANA', nil, html: {class: "fancy-nav-header", 'data-icon': "hana-icon" }, if: -> {services.available?(:bare_metal_hana,:nodes) } do |bare_metal_hana_nav|
    ...
    primary.item :storage, 'Storage', nil, html: {class: "fancy-nav-header", 'data-icon': "storage-icon" },
      ...
      storage_nav.item :shared_filesystem_storage, 'Shared File System Storage', -> {plugin('shared_filesystem_storage').start_path('shares')}, if: -> { services.available?(:shared_filesystem_storage) }, highlights_on: Proc.new { params[:controller][/shared_filesystem_storage\/.*/] }
    ...
    primary.item :monitoring, 'Monitoring, Logs, Cost Control', nil,
      ...
      if: -> {services.available?(:resource_management,:resources) or plugin_available?(:monitoring) or services.available?(:cost_control)} do |monitoring_nav|
